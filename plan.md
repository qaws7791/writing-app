...
| # | 작업 | 파일/경로 | 상세 | 상태 |
|---|---|---|---|---|
| 2-1-1 | `GET /home` 응답 수정 | `packages/core/src/modules/home/` | v2.1 홈 화면에 맞게 응답 구조 변경: ① 진행 중인 여정 카드 목록 (최우선), ② 진행 중인 여정이 없을 때 "첫 여정 시작하기" CTA 플래그, ③ "오늘의 추천 글감" 필드 제거 (홈에서 글감 미노왁), ④ "글쓰기 제안" 보조 카드 데이터(단순 boolean 또는 static) | - [ ] |
...
| # | 작업 | 파일/경로 | 상세 | 상태 |
|---|---|---|---|---|
| 2-2-1 | `GET /prompts/{id}/writings` 비활성화 | `apps/api/src/http/routes/prompts/` | v2.1에서 글감 상세 화면이 없으므로 해당 엔드포인트 deprecated 처리. 즐시 삭제하지 않고 주석/미사용 상태로 유지 가능 | - [ ] |
| 2-2-2 | `GET /prompts/{id}` 유지 | — | 에디터의 글감 참조 카드에서 아을쟁. 변경 없음 | - [ ] |
| 2-2-3 | `GET /prompts` 유지 | — | 글쓰기 진입 화면 + Bottom Sheet에서 글감 목록 조회에 사용. 변경 없음 | - [ ] |
| 2-2-4 | `PUT /prompts/{id}/bookmark` 검토 | — | v2.1 PRD에서 글감 북마크 기능 미언급. 제거 여부 확인 재난 (당장은 유지) | - [ ] |
...
| 2-3-2 | 여정 완료 → 글쓰기 연결 CTA | 여정 완료 시 클라이언트에서 처리 가능. 서버 측 변경 불땄요 | - [ ] |
...
| 2-5-2 | `bun typecheck` | 타입 에러 듀른지 확인 | - [ ] |
