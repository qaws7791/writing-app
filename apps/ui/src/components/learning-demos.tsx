...
function ClassifyDemo() {
  ...
  const itemState = (itemId: string, assignedCategoryId?: string): ClassifyState => {
    if (phase === "answering") return assignedCategoryId === undefined ? "idle" : "placed";
    ...
    >
      <Step>
        ...
        <StepBody>
          ...
          >
            <ClassifyPool>
              {CLASSIFY_ITEMS.filter((item) => placements[item.id] === undefined).map((item) => (
                <ClassifyItem id={item.id} key={item.id} state={itemState(item.id)}>
            ...
            <ClassifyBaskets>
              {CATEGORIES.map((category, index) => (
                ...
                >
                  {CLASSIFY_ITEMS.filter((item) => placements[item.id] === category.id).map(
                    (item) => (
                      <ClassifyItem
                        id={item.id}
                        key={item.id}
