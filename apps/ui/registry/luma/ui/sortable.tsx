...
function Sortable<T extends SortableValue>({
  ...
  const getItemLabelRef = React.useRef(getItemLabel);
      ...
      Accessibility.configure({
        ...
        announcements: {
          dragstart({ operation: { source } }: DragStartEvent) {
            if (!source) return;
            return `${getItemLabelRef.current(source.id as T)} 항목을 들었습니다.`;
          },
          dragover({ operation: { source, target } }: DragOverEvent) {
            if (!source || !target || !isSortable(target)) return;
            return `${getItemLabelRef.current(source.id as T)} 항목을 ${target.index + 1}번째 위치로 이동했습니다.`;
          },
          dragend({ operation: { source }, canceled }: DragEndEvent) {
            if (!source || !isSortable(source)) return;
            if (canceled) {
              return `${getItemLabelRef.current(source.id as T)} 항목 이동을 취소했습니다.`;
            }
            return `${getItemLabelRef.current(source.id as T)} 항목을 ${source.index + 1}번째 위치에 놓았습니다.`;
          },
        },
      }),
    [],
    ...
    <SortableContext.Provider value={contextValue}>
      <DragDropProvider
        ...
        onDragEnd={(event) => {
          ...
          if (disabled || event.canceled || !source || !isSortable(source)) return;
    ...
    <SortableItemContext.Provider value={itemContext}>
      <li
        ...
        className={cn(
          ...
          state === "locked" && "border-border/60 bg-muted/40 text-muted-foreground",
...
function SortableHandle({
  ...
  return (
    <button
      ...
      {...props}
    >
      <HugeiconsIcon icon={DragDropVerticalIcon} strokeWidth={1.75} className="size-4" />
    </button>
  );
}

function SortableIndex({ className, children, ...props }: React.ComponentProps<"span">) {
  const item = React.useContext(SortableItemContext);
...
export { Sortable, SortableItem, SortableHandle, SortableIndex, SortableContent };
export type { SortableProps, SortableItemProps, SortableState, SortableValue };
