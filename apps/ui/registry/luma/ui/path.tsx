...
function PathUnitHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
...
function PathUnitDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
  ...
  "group/path-node relative flex size-14 items-center justify-center rounded-full border text-sm font-medium tracking-[-0.01em] transition-[background-color,border-color,color,box-shadow,scale] duration-125 ease-press outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/25 active:scale-98 disabled:pointer-events-none",
  {
    variants: {
      state: {
        ...
        available: "border-border bg-card text-foreground shadow-xs hover:bg-accent/50",
        current:
          "border-primary/40 bg-primary text-primary-foreground shadow-sm ring-4 ring-primary/10",
        completed: "border-foreground/20 bg-foreground text-background shadow-xs",
      },
    },
    defaultVariants: {
      state: "available",
...
function PathNodeDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
...
function PathStep({ className, children, ...props }: React.ComponentProps<"li">) {
  return (
    <li data-slot="path-step" className={cn("flex flex-col items-center", className)} {...props}>
      {children}
    </li>
  );
}
