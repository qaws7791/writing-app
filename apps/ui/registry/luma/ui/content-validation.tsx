...
const CONTENT_VALIDATION_SEVERITY_LABELS: Record<ContentValidationSeverity, string> = {
  error: "오류",
  warning: "경고",
  info: "정보",
};

function ContentValidation({ className, ...props }: React.ComponentProps<"section">) {
  return (
    <section
...
function ContentValidationHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
...
function ContentValidationTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
...
function ContentValidationSummary({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
...
function ContentValidationList({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
...
const contentValidationIssueVariants = cva(
  "grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-x-2.5 gap-y-1 rounded-2xl border px-3.5 py-3",
  {
    variants: {
      severity: {
        error: "border-foreground/20 bg-foreground/[0.04] dark:bg-foreground/[0.06]",
        warning: "border-foreground/12 bg-foreground/[0.03] dark:bg-foreground/[0.05]",
...
function ContentValidationIssueTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
...
function ContentValidationIssueDetail({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
...
function ContentValidationIssueMeta({
  ...
  return (
    <p
      ...
      {...props}
    >
      {children ?? (severity ? CONTENT_VALIDATION_SEVERITY_LABELS[severity] : undefined)}
    </p>
  );
}

function ContentValidationIssueActions({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="content-validation-issue-actions"
      className={cn("col-start-3 row-span-3 flex shrink-0 items-start gap-1", className)}
      {...props}
    />
  );
}
