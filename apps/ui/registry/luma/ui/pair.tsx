...
type PairState = "idle" | "active" | "paired" | "correct" | "incorrect" | "locked";
  ...
  "group/pair-item flex w-full items-center gap-3 rounded-3xl border px-4 py-3.5 text-left text-sm font-medium tracking-[-0.01em] transition-[background-color,border-color,box-shadow,color] outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/25 disabled:pointer-events-none disabled:opacity-45",
  {
    variants: {
      state: {
        ...
        active: "border-primary/35 bg-accent/55 text-foreground shadow-2xs ring-1 ring-primary/10",
...
function PairLabel({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span data-slot="pair-label" className={cn("min-w-0 flex-1 leading-6", className)} {...props} />
  );
}
...
function getAnchor(
  ...
  return {
    x: (side === "left" ? rect.right - inset : rect.left + inset) - boardRect.left,
...
function PairLink({
  ...
  return (
    ...
    >
      {children ?? (
        <>
          <span className="min-w-0 flex-1 font-medium tracking-[-0.01em]">{leftLabel}</span>
          <span aria-hidden className="text-muted-foreground">
            —
          </span>
          <span className="min-w-0 flex-1 text-right text-muted-foreground">{rightLabel}</span>
        </>
      )}
    </div>
  );
...
export type { PairState, PairConnection };
