...
function CadenceHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
...
function CadenceWeek({ className, ...props }: React.ComponentProps<"ol">) {
  return (
    <ol data-slot="cadence-week" className={cn("grid grid-cols-7 gap-2", className)} {...props} />
  );
}

const cadenceDayVariants = cva("flex flex-col items-center gap-1.5 text-center outline-none", {
  variants: {
    state: {
      practiced: "",
      rest: "",
  ...
  "flex size-8 items-center justify-center rounded-full border transition-[background-color,border-color,color,box-shadow] duration-125 [&>svg]:size-3.5",
  {
    variants: {
      state: {
        ...
        today: "border-primary/40 bg-primary/10 text-foreground ring-3 ring-primary/12",
...
function CadenceDayMark({ state }: { state: CadenceDayState }) {
  switch (state) {
    case "practiced":
      return <HugeiconsIcon icon={Tick02Icon} strokeWidth={2.25} aria-hidden="true" />;
    case "rest":
      return <HugeiconsIcon icon={MinusSignIcon} strokeWidth={2} aria-hidden="true" />;
    case "today":
      return <span aria-hidden="true" className="size-1.5 rounded-full bg-foreground/55" />;
    case "upcoming":
    default:
      return null;
  }
