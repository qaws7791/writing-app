...
const LEARNER_PREVIEW_PERSONA_LABELS: Record<LearnerPreviewPersonaKind, string> = {
  novice: "초급",
  fluent: "숙련",
};

const LEARNER_PREVIEW_SCENARIO_LABELS: Record<LearnerPreviewScenarioKind, string> = {
  correct: "정답",
  incorrect: "오답",
  offline: "오프라인",
};

function LearnerPreview({ className, ...props }: React.ComponentProps<"section">) {
  return (
    <section
...
function LearnerPreviewHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
...
function LearnerPreviewTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
...
function LearnerPreviewToolbar({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
  ...
  "inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-medium tracking-[0.02em] transition-colors",
  {
    variants: {
      active: {
        ...
        false: "border-border/70 bg-card text-muted-foreground hover:bg-muted/40",
      },
    },
    defaultVariants: {
      active: false,
...
function LearnerPreviewFrame({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
