...
type ClassifyState = "idle" | "active" | "placed" | "correct" | "incorrect" | "locked";

function Classify({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="classify" className={cn("flex w-full flex-col gap-6", className)} {...props} />
  );
}

function ClassifyCategories({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      role="toolbar"
      aria-label="카테고리"
  ...
  "inline-flex h-9 items-center justify-center rounded-full border px-3.5 text-sm font-medium tracking-[-0.005em] transition-[background-color,border-color,color,box-shadow] outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/25 disabled:pointer-events-none disabled:opacity-45",
  {
    variants: {
      state: {
        ...
        active: "border-primary/35 bg-accent/60 text-foreground shadow-2xs ring-1 ring-primary/10",
...
function ClassifyItemLabel({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      data-slot="classify-item-label"
      className={cn("min-w-0 flex-1 leading-6 font-medium tracking-[-0.01em]", className)}
      {...props}
    />
  );
}

function ClassifyItemTag({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
