...
type ChoiceState = "idle" | "selected" | "correct" | "incorrect" | "missed" | "locked";
  ...
  "relative flex w-full items-start rounded-3xl border px-4 py-3.5 text-left text-sm transition-[background-color,border-color,box-shadow,color] outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/25 disabled:pointer-events-none disabled:opacity-45",
  {
    variants: {
      state: {
        ...
        incorrect: "border-destructive/30 bg-destructive/6 text-destructive shadow-none",
        missed: "border-dashed border-border bg-transparent text-muted-foreground shadow-none",
        locked: "border-border/60 bg-muted/40 text-muted-foreground shadow-none",
      },
    },
    defaultVariants: {
      state: "idle",
...
function Choice({
  ...
  return (
    <button
      ...
      aria-checked={selected || resolvedState === "correct" || resolvedState === "incorrect"}
      data-slot="choice"
...
function ChoiceLabel({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      ...
      className={cn("text-sm leading-6 font-medium tracking-[-0.01em]", className)}
      {...props}
    />
  );
}

function ChoiceDescription({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
...
export { Choice, ChoiceGroup, ChoiceContent, ChoiceLabel, ChoiceDescription, choiceVariants };
