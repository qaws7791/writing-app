...
function SubmissionHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
  ...
  "inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-medium tracking-[0.02em]",
  {
    variants: {
      state: {
        ...
        "in-review": "border-foreground/15 bg-foreground/[0.04] text-foreground/80",
        "revision-requested": "border-destructive/30 bg-destructive/5 text-destructive",
...
function SubmissionActions({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
