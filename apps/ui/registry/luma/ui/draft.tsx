...
type DraftStatusValue = "editing" | "saving" | "saved" | "offline" | "submittable" | "submitted";
  ...
  "inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-medium tracking-[0.02em]",
  {
    variants: {
      status: {
        ...
        submittable: "border-foreground/15 bg-foreground/[0.04] text-foreground/80",
...
function DraftEditor({ className, ...props }: React.ComponentProps<typeof Textarea>) {
  return (
    <Textarea
      data-slot="draft-editor"
      className={cn("min-h-56 rounded-3xl text-base leading-7 md:min-h-72 md:text-base", className)}
      {...props}
    />
  );
}
...
function DraftMeter({
  ...
  return (
    ...
    >
      <span
        className={cn("font-medium", meetsMin ? "text-foreground/80" : "text-muted-foreground")}
      >
        {characters.toLocaleString("ko-KR")}자
      </span>
      <span>{paragraphs.toLocaleString("ko-KR")}문단</span>
      {minCharacters !== undefined && <span>최소 {minCharacters.toLocaleString("ko-KR")}자</span>}
    </div>
  );
}
