...
function StepPrompt({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn("text-base leading-7 text-pretty text-muted-foreground", className)}
      {...props}
    />
  );
}
...
function StepBody({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="step-body" className={cn("flex flex-col gap-6", className)} {...props} />;
}
