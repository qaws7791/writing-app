...
const MASTERY_LEVELS: MasteryLevel[] = ["emerging", "developing", "secure", "fluent"];
...
function MasteryHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
  ...
  return (
    ...
    >
      {MASTERY_LEVELS.map((stage, index) => {
        const tone = index < activeIndex ? "filled" : index === activeIndex ? "current" : "empty";
        return (
          <span
            key={stage}
...
function MasteryDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
