...
function StandingHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
...
function StandingList({ className, ...props }: React.ComponentProps<"ol">) {
  return (
    <ol data-slot="standing-list" className={cn("flex flex-col gap-0.5", className)} {...props} />
  );
}
...
function StandingName({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("min-w-0 truncate font-medium tracking-[-0.01em]", className)}
      {...props}
    />
  );
}
