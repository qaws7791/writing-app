...
type RunQueueOutcomeKind = "on-track" | "needs-approval" | "retrying" | "escalated" | "done";
...
function RunQueue({ className, ...props }: React.ComponentProps<"section">) {
  return (
    <section
      data-slot="run-queue"
      className={cn("@container/run-queue flex w-full flex-col gap-4", className)}
      {...props}
    />
  );
}

function RunQueueHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
...
function RunQueueSummaryChip({
  ...
  tone = "default",
  ...props
}: React.ComponentProps<"li"> & VariantProps<typeof runQueueSummaryChipVariants>) {
  return (
    <li
...
function RunQueueGroups({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="run-queue-groups" className={cn("flex flex-col gap-5", className)} {...props} />
  );
}
...
function RunQueueGroupHeader({ className, ...props }: React.ComponentProps<"header">) {
  return (
    <header
      data-slot="run-queue-group-header"
      className={cn("flex items-center justify-between gap-3 px-0.5", className)}
      {...props}
    />
  );
}
...
function RunQueueGroupTitle({
  ...
  return (
    <div
      ...
      className={cn("flex items-center gap-2 text-sm font-medium tracking-[-0.01em]", className)}
      {...props}
    >
...
function RunQueueGroupCount({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
...
function RunQueueList({ className, ...props }: React.ComponentProps<"ol">) {
  return (
    <ol data-slot="run-queue-list" className={cn("flex flex-col gap-1.5", className)} {...props} />
  );
}
...
function RunQueueItemIcon({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
...
function RunQueueItemBody({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
...
function RunQueueItemTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="run-queue-item-title"
      className={cn("truncate text-sm font-medium tracking-[-0.01em]", className)}
      {...props}
    />
  );
}
...
const runQueueEnvironmentVariants = cva(
  "inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2 py-0.5 text-[10px] font-medium tracking-[-0.01em]",
  {
    variants: {
      environment: {
        production: "border-foreground/15 bg-foreground/[0.04] text-foreground/80",
...
function RunQueueItemTime({ className, ...props }: React.ComponentProps<"time">) {
  return (
    <time
...
function RunQueueFooter({ className, ...props }: React.ComponentProps<"footer">) {
  return (
    <footer
