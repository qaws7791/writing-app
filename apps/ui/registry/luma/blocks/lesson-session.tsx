...
import { Prose, ProseBody } from "@/registry/luma/ui/prose";
        ...
        <StepBody>
          <Prose>
            <ProseBody>
              <p>
                ...
                만드는 재료입니다. 둘이 멀어지면 글은 길게 쓰여도 설득력이 약해집니다.
              </p>
              <p>
                좋은 근거는 주장을 반복하지 않고, 독자가 “왜?”라고 물을 자리에 사실·사례·원리를
                둡니다.
              </p>
            </ProseBody>
          </Prose>
          {acked ? (
            <Insight tone="think">
              ...
              <InsightDescription>
                다음 활동부터는 주장·근거를 고르고, 연결하고, 짧은 글로 써 봅니다.
              </InsightDescription>
            </Insight>
          ) : null}
        </StepBody>
...
function CategorizeStep({
  ...
  const [activeCategory, setActiveCategory] = React.useState<string | null>("claim");
      ...
      <Step>
        ...
        <StepBody>
          <Compose>
            ...
            />
            <ComposeMeter value={value.length} min={WRITE_MIN} goal={WRITE_GOAL} max={WRITE_MAX} />
          </Compose>
          {submitted ? (
...
function LessonFlowStep({
  ...
  switch (kind) {
    case "READING":
      return <ReadingStep stepIndex={stepIndex} onComplete={onComplete} onClose={onClose} />;
    case "MULTIPLE_CHOICE":
      return <ChoiceStep stepIndex={stepIndex} onComplete={onComplete} onClose={onClose} />;
    case "FILL_BLANK":
      return <TokenStep stepIndex={stepIndex} onComplete={onComplete} onClose={onClose} />;
    case "SELECT":
      return <SegmentStep stepIndex={stepIndex} onComplete={onComplete} onClose={onClose} />;
    case "ORDER":
      return <OrderStep stepIndex={stepIndex} onComplete={onComplete} onClose={onClose} />;
    case "MATCH":
      return <MatchStep stepIndex={stepIndex} onComplete={onComplete} onClose={onClose} />;
    case "CATEGORIZE":
      return <CategorizeStep stepIndex={stepIndex} onComplete={onComplete} onClose={onClose} />;
    case "COMPARE":
      return <CompareStep stepIndex={stepIndex} onComplete={onComplete} onClose={onClose} />;
    case "WRITE":
      return (
        <WriteStep
          stepIndex={stepIndex}
        ...
        <Lesson className="min-h-0 w-full flex-1 px-4 pt-4 sm:px-6">
          ...
          <LessonBody>
            <LessonComplete>
              ...
              <LessonCompleteDescription>
                주장과 근거를 읽고, 고르고, 연결하고, 쓰는 열 가지 활동을 모두 둘러보았습니다. 다시
                시작하면 처음부터 연습할 수 있습니다.
              </LessonCompleteDescription>
            </LessonComplete>
          </LessonBody>
          <LessonFooter>
