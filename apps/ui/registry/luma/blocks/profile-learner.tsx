...
import { LearnerShell, type LearnerProfileAction } from "@/registry/luma/blocks/learner-shell";
...
function parseViewHash(): ProfileView {
  ...
  if (hash === "history" || hash === "settings" || hash === "profile") return hash;
  return "profile";
}
...
function SubpageHeader({
  ...
  return (
    <header data-slot="profile-learner-subpage-header" className="flex items-center gap-2">
      <Button
        type="button"
        variant="ghost"
        size="icon-sm"
...
function ProfileRoot({
  ...
  return (
    <div data-slot="profile-learner-root" className="@container flex flex-col gap-12 sm:gap-14">
      ...
      <NextAction className="rounded-[1.75rem] bg-muted/55 px-5 py-5 sm:px-6 sm:py-6">
        ...
        <NextActionBody>
          ...
          <NextActionReason>
            주장 세우기는 안정 단계입니다. 근거를 스스로 고르는 연습을 이어 가면 흐름이 유지됩니다.
          </NextActionReason>
        ...
        <ItemGroup className="gap-2">
          ...
          >
            ...
            <ItemContent>
              ...
              <ItemDescription>완료한 활동, 연습 추천, 반복 오류를 봅니다.</ItemDescription>
            </ItemContent>
            <ItemActions>
              <HugeiconsIcon
          ...
          >
            ...
            <ItemContent>
              ...
              <ItemDescription>표시 이름과 학습 선호를 조정합니다.</ItemDescription>
            </ItemContent>
            <ItemActions>
              <HugeiconsIcon
                icon={ArrowRight01Icon}
...
function HistoryView({
  ...
  return (
    <div data-slot="profile-learner-history" className="flex flex-col gap-10">
      ...
      <section className="flex flex-col gap-4" aria-labelledby="profile-history-activity">
        <header className="flex items-baseline justify-between gap-3">
          <h2 id="profile-history-activity" className="text-sm font-medium tracking-[-0.01em]">
            최근 활동
          </h2>
...
function SettingsView({
  ...
  return (
    <div data-slot="profile-learner-settings" className="flex flex-col gap-10">
      ...
      <section className="flex flex-col gap-4" aria-labelledby="profile-settings-account">
        <h2 id="profile-settings-account" className="text-sm font-medium tracking-[-0.01em]">
          계정
        </h2>
        ...
        {editingPrefs ? (
          <>
              ...
              <LearningProfileOptions mode="single">
                {["논증 글쓰기", "학술 에세이", "설득 카피"].map((option) => (
                  <LearningProfileOption
                    ...
                    onClick={() =>
                      setDraft((current) => (current ? { ...current, purpose: option } : current))
                    }
                  >
                    {option}
                  </LearningProfileOption>
              ...
              <LearningProfileOptions mode="single">
                {["입문", "중급", "숙련"].map((option) => (
                  <LearningProfileOption
                    ...
                    onClick={() =>
                      setDraft((current) => (current ? { ...current, level: option } : current))
                    }
                  >
                    {option}
                  </LearningProfileOption>
            ...
            <LearningProfileSection>
              ...
              <LearningProfileOptions mode="single">
                {["주 1시간 이하", "주 2–3시간", "주 4시간 이상"].map((option) => (
                  <LearningProfileOption
                    key={option}
                    mode="single"
                    ...
                    onClick={() =>
                      setDraft((current) =>
                        current ? { ...current, weeklyHours: option } : current,
                      )
                    }
                  >
                    {option}
              ...
              <LearningProfileOptions mode="single">
                {["힌트만", "힌트 후 코칭", "바로 코칭"].map((option) => (
                  <LearningProfileOption
                    ...
                    onClick={() =>
                      setDraft((current) => (current ? { ...current, feedback: option } : current))
                    }
                  >
                    {option}
                  </LearningProfileOption>
            ...
            <LearningProfileFooter>
              <Button type="button" variant="outline" onClick={cancelEditingPrefs}>
                취소
              </Button>
          ...
          <>
            <LearningProfileSummary>
              <LearningProfileSummaryRow>
                ...
                <LearningProfileSummaryValue>{purpose}</LearningProfileSummaryValue>
              </LearningProfileSummaryRow>
              <LearningProfileSummaryRow>
                <LearningProfileSummaryTerm>수준</LearningProfileSummaryTerm>
                <LearningProfileSummaryValue>{level}</LearningProfileSummaryValue>
              </LearningProfileSummaryRow>
              <LearningProfileSummaryRow>
              ...
              <LearningProfileSummaryRow>
                ...
                <LearningProfileSummaryValue>{weeklyHours}</LearningProfileSummaryValue>
              </LearningProfileSummaryRow>
              <LearningProfileSummaryRow>
                <LearningProfileSummaryTerm>피드백</LearningProfileSummaryTerm>
                <LearningProfileSummaryValue>{feedback}</LearningProfileSummaryValue>
              </LearningProfileSummaryRow>
            </LearningProfileSummary>
            <LearningProfileFooter>
              <Button type="button" variant="outline" onClick={startEditingPrefs}>
                선호 수정
              </Button>
            </LearningProfileFooter>
          </>
 ...
 */
export function ProfileLearner({ className, ...props }: React.ComponentProps<"div">) {
