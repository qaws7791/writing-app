...
import {
  ...
  ProvenanceRowModel,
  ProvenanceRowStatus,
} from "@/registry/luma/ui/provenance-panel";
import {
  ...
  type RunQueueStatus,
} from "@/registry/luma/ui/run-queue";
import {
  Select,
  SelectContent,
  SelectItem,
...
} from "@/registry/luma/ui/step-trace";
...
const STATUS_ORDER: RunQueueStatus[] = ["running", "waiting", "failed", "completed"];
...
const RUNS: AgentRun[] = [
    ...
    steps: [
      {
        ...
        status: "completed",
        duration: "14s",
        tools: [{ name: "orders.lookup", duration: "420ms", status: "succeeded" }],
      },
      {
        id: "s2",
        title: "원거래와 중복 결제 대조",
        status: "completed",
        duration: "31s",
        tools: [{ name: "payments.match", duration: "1.2s", status: "succeeded" }],
      },
      {
        id: "s3",
        title: "부분 캡처 환불 요청",
        ...
        tools: [{ name: "payments.refunds.create", duration: "890ms", status: "failed" }],
      },
      {
        id: "s4",
        title: "대체 환불 경로 제안",
    ...
    steps: [
      {
        ...
        status: "completed",
        duration: "9s",
        tools: [{ name: "risk.signals", duration: "310ms", status: "succeeded" }],
      },
      {
        id: "s2",
        title: "기기 지문 확인",
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "s4", title: "웨어하우스 적재", status: "completed", duration: "2m 03s" },
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "s2", title: "환불 금액 산정", status: "completed", duration: "11s" },
  ...
  },
  {
    ...
    model: "hermes-fraud-v1",
    steps: [
      { id: "s1", title: "후보 목록 로드", status: "completed", duration: "7s" },
      { id: "s2", title: "히스토리 조인", status: "completed", duration: "16s" },
      {
        id: "s3",
        title: "차단 규칙 평가",
        status: "running",
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "s2", title: "주제 클러스터링", status: "completed", duration: "41s" },
...
function SeverityBadge({ severity }: { severity: Incident["severity"] }) {
  ...
  const variant =
    severity === "critical" ? "destructive" : severity === "major" ? "warning" : "info";
 ...
 */
export function AgentMissionControl({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const byStatus = Object.fromEntries(STATUS_ORDER.map((status) => [status, 0])) as Record<
    RunQueueStatus,
    number
  >;
  ...
  const counts = {
    ...
    needsApproval: filtered.filter((run) => run.outcome === "needs-approval").length,
    unassigned: 0,
    byStatus,
  };
      ...
      >
        ...
        <RunQueueSummary aria-label="실행 요약">
          ...
          <RunQueueSummaryChip tone={counts.escalated ? "danger" : "default"}>
            Escalated <span className="text-foreground">{counts.escalated}</span>
          </RunQueueSummaryChip>
          <RunQueueSummaryChip tone={counts.needsApproval ? "warning" : "default"}>
            Needs approval <span className="text-foreground">{counts.needsApproval}</span>
          </RunQueueSummaryChip>
          <RunQueueSummaryChip>
            Unassigned <span className="text-foreground">{counts.unassigned}</span>
          </RunQueueSummaryChip>
        </RunQueueSummary>
      </section>
      ...
      <RunQueue data-slot="agent-mission-control-queue">
        ...
        <RunQueueToolbar>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <SelectTrigger size="sm" className="w-full sm:w-36" aria-label="상태 필터">
                <SelectValue />
              </SelectTrigger>
          ...
          <RunQueueEmpty>
            <Empty variant="compact">
              <EmptyHeader>
                ...
                <EmptyDescription>검색어나 상태 필터를 넓혀 다시 살펴보세요.</EmptyDescription>
              </EmptyHeader>
            </Empty>
          </RunQueueEmpty>
        ) : (
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <RunQueueEnvironment environment={run.environment} />
                            <RunQueueItemTime dateTime={run.updatedAt} className="inline">
                              {run.updatedLabel}
                            </RunQueueItemTime>
      ...
      >
        ...
        >
          <header className="flex items-baseline justify-between gap-3 px-0.5">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">최근 24시간</p>
            </div>
          ...
          >
            <AreaChart data={[...THROUGHPUT]} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
              <defs>
                <linearGradient id="agent-mission-control-runs" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="var(--color-runs)" stopOpacity={0.28} />
        ...
        >
          <header className="flex items-baseline justify-between gap-3">
            ...
            <p className="text-xs tabular-nums text-muted-foreground">{INCIDENTS.length}건</p>
          </header>
        ...
        <SheetContent className="flex w-full flex-col gap-0 sm:max-w-xl" side="right">
          ...
          {selected ? (
            <div className="flex flex-1 flex-col gap-6 overflow-auto p-6">
              ...
              <p className="text-sm text-pretty text-muted-foreground">{selected.title}</p>
              ...
              <div className="grid gap-6">
                    ...
                    {selected.steps.map((step, index) => (
                      <StepTraceStep key={step.id} status={step.status}>
                        ...
                        <StepTraceBody>
                              ...
                              {step.tools.map((tool) => (
                                <StepTraceTool key={tool.name}>
                                  ...
                                  <div className="flex items-center gap-2">
                                    <StepTraceToolDuration>{tool.duration}</StepTraceToolDuration>
                          ...
                          {step.error ? <StepTraceError>{step.error}</StepTraceError> : null}
                        </StepTraceBody>
                      </StepTraceStep>
                    ))}
                  </StepTraceList>
                ...
                <section className="flex flex-col gap-4 rounded-2xl border border-border/70 bg-card p-4">
                  ...
                  <dl className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,10rem),1fr))] gap-4">
                    <DetailRow label="상태">{RUN_QUEUE_STATUS_LABELS[selected.status]}</DetailRow>
                    ...
                    <DetailRow label="재시도 정책">{selected.retryPolicy}</DetailRow>
                    <DetailRow label="최대 재시도">{selected.maxRetries}</DetailRow>
                    ...
                    <DetailRow label="최근 활동">{selected.lastActivity}</DetailRow>
                  </dl>
                </section>
                ...
                <ProvenancePanel>
                  ...
                  <ProvenanceList>
                    <ProvenanceRow source="ai" verified={selected.status !== "failed"}>
                      <ProvenanceRowLabel source="ai" />
                      <ProvenanceRowMeta>{selected.agent} 자동 실행</ProvenanceRowMeta>
                      <ProvenanceRowModel>{selected.model}</ProvenanceRowModel>
                      <ProvenanceRowStatus verified={selected.status !== "failed"}>
