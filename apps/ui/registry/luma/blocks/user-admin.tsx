...
import {
  ...
  CohortTargetLabel,
  CohortTargetTitle,
} from "@/registry/luma/ui/cohort-assignment";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
        ...
        <div className="@container grid gap-5 @[32rem]:grid-cols-2">
          <Field>
            ...
            <Select
              ...
              onValueChange={(value) => {
                ...
                if (next === "learner" || next === "instructor") onRoleChange(next);
              }}
            >
          ...
          <Field>
            ...
            <Select
              ...
              onValueChange={(value) => {
                const next = Array.isArray(value) ? value[0] : value;
                if (next === "active" || next === "invited" || next === "suspended") {
                  onStatusChange(next);
                }
              }}
            >
    ...
    >
      <LearnerRecord className="rounded-[1.25rem] border border-border/70 bg-card p-4 sm:p-5">
        ...
        <LearnerRecordSection>
          ...
          <LearnerRecordSubmissions>{USER.submissionsLabel}</LearnerRecordSubmissions>
        </LearnerRecordSection>
        <LearnerRecordSection>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-4 rounded-[1.25rem] border border-border/70 bg-card p-4 sm:p-5">
          <Mastery level="fluent">
            ...
            <MasteryDescription>근거를 스스로 고르고 문단에 붙이는 단계입니다.</MasteryDescription>
          </Mastery>
          <Mastery level="developing">
            ...
            <MasteryDescription>격식 전환 연습이 이어지고 있습니다.</MasteryDescription>
          </Mastery>
        </div>
 ...
 */
export function UserAdmin({ className, ...props }: React.ComponentProps<"div">) {
      ...
      >
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-2">
          ...
          <DropdownMenu>
            ...
            >
              <HugeiconsIcon icon={MoreHorizontalIcon} strokeWidth={2} data-icon="inline-start" />
              작업
            </DropdownMenuTrigger>
        ...
        <TabsContent value="record" className="min-h-0 overflow-auto outline-none">
          <RecordPanel />
        </TabsContent>

        <TabsContent value="support" className="min-h-0 overflow-auto outline-none">
          <SupportPanel />
        </TabsContent>
      </Tabs>
      ...
      <AlertDialog open={suspendOpen} onOpenChange={setSuspendOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {USER.name}님의 학습 접근이 즉시 막힙니다. 진행 기록은 유지되며, 나중에 재활성화할 수
              있습니다. 이 변경은 감사 로그에 남습니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>취소</AlertDialogCancel>
            <AlertDialogAction variant="destructive" onClick={handleSuspendConfirm}>
              일시정지
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
