...
import { BookOpen01Icon, Calendar01Icon, PlayIcon } from "@hugeicons/core-free-icons";

import { cn } from "@/registry/luma/lib/utils";
import { LearnerShell } from "@/registry/luma/blocks/learner-shell";
import { Card, cardVariants } from "@/registry/luma/ui/card";
...
function InProgressCourseCard({ course }: { course: CourseProgress }) {
  ...
  return (
    <article
      ...
      data-size="sm"
      className={cn(cardVariants({ variant: "surface", size: "sm" }), "gap-5 rounded-[1.75rem]")}
    >
      ...
      <div className="px-(--card-spacing)">
        ...
        >
          <span className="grid size-10 shrink-0 place-items-center rounded-full bg-foreground text-background">
            <HugeiconsIcon icon={PlayIcon} strokeWidth={2} className="size-4 translate-x-px" />
          </span>
      ...
      <div className="flex w-full items-center gap-4 px-(--card-spacing)">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[11px] font-medium tracking-[0.04em] text-muted-foreground uppercase">
            ...
            <span className="tabular-nums normal-case tracking-normal">완료</span>
          </p>
 ...
 */
export function HomeLearner({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <LearnerShell data-slot="home-learner" className={className} currentNav="home" {...props}>
      <main className="@container mx-auto grid w-full max-w-5xl flex-1 grid-cols-1 gap-10 px-5 py-10 sm:px-8 sm:py-12 @[48rem]/learner-shell:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] @[48rem]/learner-shell:items-start @[48rem]/learner-shell:gap-12">
        ...
        <section data-slot="home-learner-courses" aria-label="학습 진행">
          <Tabs defaultValue="in-progress" className="gap-4">
            ...
            <TabsContent value="in-progress" className="flex flex-col gap-3 outline-none">
              {IN_PROGRESS.map((course) => (
                <InProgressCourseCard key={course.id} course={course} />
              ))}
            </TabsContent>

            <TabsContent value="completed" className="flex flex-col gap-3 outline-none">
              {COMPLETED.length > 0 ? (
                COMPLETED.map((course) => <CompletedCourseCard key={course.id} course={course} />)
              ) : (
                <Card
                  variant="muted"
                  size="sm"
