 ...
 */
export function LoginWorkspace({ className, ...props }: React.ComponentProps<"div">) {
          ...
          <div className="relative flex h-full flex-col justify-between p-12">
            ...
            <div className="max-w-md">
              ...
              <p className="mt-4 text-sm leading-7 text-muted-foreground">
                조직 도메인과 SSO를 먼저 확인한 뒤, 구성원 계정으로 이어집니다. 개인 비밀번호보다
                회사 정책을 앞세웁니다.
              </p>
              <dl className="mt-10 grid gap-5 text-sm">
                <div>
                  <dt className="font-medium tracking-[-0.01em]">단일 로그인</dt>
                  <dd className="mt-1 text-muted-foreground">Okta, Google Workspace, Azure AD</dd>
                </div>
                <div>
                  <dt className="font-medium tracking-[-0.01em]">역할 기반 접근</dt>
                  <dd className="mt-1 text-muted-foreground">편집자, 검토자, 관리자 권한 유지</dd>
                </div>
              </dl>
            </div>
          ...
          <div className="mx-auto w-full max-w-[23rem]">
            ...
            {mode === "workspace" ? (
              <form className="flex flex-col gap-6" onSubmit={(event) => event.preventDefault()}>
                <FieldGroup>
                  <Field>
                    <FieldLabel htmlFor="login-workspace-slug">워크스페이스 URL</FieldLabel>
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        required
                      />
                      <span className="shrink-0 text-sm text-muted-foreground">.luma.app</span>
                    </div>
                    <FieldDescription>관리자에게 받은 팀 주소를 입력하세요.</FieldDescription>
                  </Field>
                </FieldGroup>
              ...
              <form className="flex flex-col gap-6" onSubmit={(event) => event.preventDefault()}>
                <FieldGroup>
                  <Field>
                    <FieldLabel htmlFor="login-workspace-sso-email">회사 이메일</FieldLabel>
                    <Input
                      ...
                      required
                    />
                    <FieldDescription>등록된 도메인이면 회사 IdP로 이동합니다.</FieldDescription>
                  </Field>
                </FieldGroup>
                <Button type="submit" size="lg" className="w-full">
                  SSO로 계속
