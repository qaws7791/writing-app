...
const STATUS_BADGE_VARIANT: Record<CourseStatus, "outline" | "info" | "success"> = {
  draft: "outline",
  preview: "info",
  live: "success",
};
...
const columns: ColumnDef<CourseRow>[] = [
    ...
    cell: ({ row }) => (
      <div className="flex min-w-0 max-w-[18rem] items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="truncate font-medium tracking-[-0.01em]">{row.original.title}</p>
          <p className="truncate text-xs text-muted-foreground tabular-nums">{row.original.id}</p>
        </div>
      </div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-muted-foreground">
        {row.original.topic}
        <span className="text-muted-foreground/70"> · {row.original.level}</span>
      </span>
    ),
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-right tabular-nums text-muted-foreground">{row.original.lessons}</div>
    ),
  },
  {
  ...
  },
  {
    ...
    header: "수정일",
    cell: ({ row }) => (
      <span className="tabular-nums text-muted-foreground">{row.original.updatedAt}</span>
    ),
  },
  {
    id: "actions",
...
function CreateCourseDialog({
  ...
  const [values, setValues] = React.useState<CreateCourseValues>(EMPTY_CREATE_VALUES);
  ...
  function updateField<K extends keyof CreateCourseValues>(key: K, value: CreateCourseValues[K]) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg" showCloseButton initialFocus={titleInputRef}>
        <DialogHeader className="pe-8">
          <DialogTitle>코스 만들기</DialogTitle>
          <DialogDescription>
            목록에서 구분할 기본 정보만 넣습니다. 유닛·레슨·스텝은 만든 뒤 편집에서 이어갑니다.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <FieldGroup className="gap-5">
            ...
            <Field>
              <FieldLabel htmlFor="create-course-description">한줄 설명</FieldLabel>
              <Textarea
 ...
 */
export function CoursesAdmin({ className, ...props }: React.ComponentProps<"div">) {
