...
} from "@/registry/luma/ui/intervention-queue";
import {
  ItemAnalysis,
  ...
  ItemAnalysisStats,
  ItemAnalysisTitle,
} from "@/registry/luma/ui/item-analysis";
import {
  LearningAnalytics,
...
} from "@/registry/luma/ui/learning-analytics";
import {
  Select,
  SelectContent,
  SelectItem,
...
} from "@/registry/luma/ui/writing-analytics";
...
const METRICS_BY_PERIOD: Record<PeriodId, MetricFixture[]> = {
  "7d": [
    { id: "completion", label: "완료율", value: "68%", hint: "전 기간 대비 +4%p" },
    { id: "session", label: "중앙 세션 시간", value: "14분", hint: "전 기간 대비 −1분" },
    { id: "return", label: "복습 복귀율", value: "41%", hint: "전 기간 대비 +2%p" },
    { id: "dropout", label: "이탈 레슨", value: "6", hint: "전 기간 대비 −2건" },
  ],
  "30d": [
    { id: "completion", label: "완료율", value: "72%", hint: "전 기간 대비 +3%p" },
    { id: "session", label: "중앙 세션 시간", value: "16분", hint: "전 기간 대비 +1분" },
    { id: "return", label: "복습 복귀율", value: "38%", hint: "전 기간 대비 −1%p" },
    { id: "dropout", label: "이탈 레슨", value: "9", hint: "전 기간 대비 +1건" },
  ],
  "90d": [
    { id: "completion", label: "완료율", value: "74%", hint: "전 기간 대비 +5%p" },
    { id: "session", label: "중앙 세션 시간", value: "15분", hint: "전 기간 대비 동일" },
    { id: "return", label: "복습 복귀율", value: "36%", hint: "전 기간 대비 −3%p" },
    { id: "dropout", label: "이탈 레슨", value: "14", hint: "전 기간 대비 +3건" },
  ],
};

const CHART_SUMMARY_BY_PERIOD: Record<PeriodId, string> = {
  "7d": "레슨 완료가 전 기간 대비 8% 올랐고, 주말 세션이 평일 대비 22% 낮습니다.",
  "30d": "월중 완료가 안정적이며, 주말마다 세션이 평일 대비 약 20% 낮게 유지됩니다.",
  ...
  return (
    ...
    >
      <header className="flex flex-wrap items-baseline justify-between gap-3 px-0.5">
        <div>
          <h2 id="admin-analytics-chart-title" className="text-sm font-medium tracking-[-0.01em]">
            완료·세션 추이
          </h2>
          <p className="mt-0.5 text-xs text-muted-foreground">{PERIOD_LABELS[period]}</p>
        </div>
        <p className="text-xs tabular-nums text-muted-foreground">
          레슨 완료 합계 {new Intl.NumberFormat("ko-KR").format(totalCompletions)}
        </p>
      </header>
      ...
      >
        <AreaChart data={[...data]} margin={{ left: 0, right: 8, top: 8, bottom: 0 }}>
          <defs>
            <linearGradient id="admin-analytics-completions" x1="0" y1="0" x2="0" y2="1">
            ...
            <linearGradient id="admin-analytics-sessions" x1="0" y1="0" x2="0" y2="1">
      ...
      <LearningAnalyticsSeries>
        {SERIES_ROWS.map((row) => (
          <LearningAnalyticsRow key={row.id}>
            ...
            <span className="tabular-nums text-muted-foreground">{row.dropout}</span>
...
function WritingPanel() {
  return (
    <WritingAnalytics data-slot="admin-analytics-writing">
      ...
      <WritingAnalyticsHint>
        근거 적절성 점수가 낮습니다. 「근거 고르기」 레슨과 모범 답안을 먼저 점검하세요.
      </WritingAnalyticsHint>
    </WritingAnalytics>
  );
}
...
function ItemPanel() {
  return (
    <ItemAnalysis data-slot="admin-analytics-items">
      ...
      <ItemAnalysisList>
        <ItemAnalysisRow>
          <ItemAnalysisPrompt>다음 대화에서 알맞은 존댓말 표현을 고르세요.</ItemAnalysisPrompt>
          <ItemAnalysisStats>
            <ItemAnalysisStat>
        ...
        <ItemAnalysisRow>
          <ItemAnalysisPrompt>밑줄 친 부분의 의미를 가장 잘 나타낸 것은?</ItemAnalysisPrompt>
          <ItemAnalysisStats>
            <ItemAnalysisStat>
              <ItemAnalysisStatLabel>정답률</ItemAnalysisStatLabel>
        ...
        <ItemAnalysisRow>
          <ItemAnalysisPrompt>주장에 대한 반박으로 가장 적절한 문장을 고르세요.</ItemAnalysisPrompt>
          <ItemAnalysisStats>
            <ItemAnalysisStat>
              <ItemAnalysisStatLabel>정답률</ItemAnalysisStatLabel>
    ...
    >
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger size="sm" className="w-full sm:w-36" aria-label="기간">
              <SelectValue />
            </SelectTrigger>
          ...
          >
            <SelectTrigger size="sm" className="w-full sm:w-40" aria-label="코호트">
              <SelectValue />
            </SelectTrigger>
            <SelectContent align="start">
        ...
        <LearningAnalyticsGrid>
          {metrics.map((metric) => (
            <LearningAnalyticsMetric key={metric.id}>
              <LearningAnalyticsMetricLabel>{metric.label}</LearningAnalyticsMetricLabel>
              <LearningAnalyticsMetricValue>{metric.value}</LearningAnalyticsMetricValue>
              <LearningAnalyticsMetricHint>{metric.hint}</LearningAnalyticsMetricHint>
            </LearningAnalyticsMetric>
          ))}
        </LearningAnalyticsGrid>
      </LearningAnalytics>
      ...
      <Tabs defaultValue="learning" className="gap-4" data-slot="admin-analytics-tabs">
