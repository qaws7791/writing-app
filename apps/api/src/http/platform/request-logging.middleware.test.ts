...
import type { RequestLogEvent } from "@/observability/request-logger"
import type { SecurityAuditEvent } from "@/observability/security-audit-logger"
...
describe("Hono request logging middleware", () => {
  ...
  it("owner 변경·인증 실패·AI quota를 민감 필드 없는 감사 이벤트로 남긴다", async () => {
    ...
    await ownerApp.request("/users/user-1/status?password=query-secret", {
      body: JSON.stringify({
