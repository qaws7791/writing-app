...
import {
  parseIntegerEtag,
  toIntegerEtag,
} from "@/modules/admin-resource-library/admin-integer-etag"
...
function createUploadResourceImageRoute({
  ...
  const handler: AdminRouteHandler<typeof routeConfig> = async (context) => {
    ...
    const document = adminResourceDocumentDtoSchema.parse(documentResult)
    if (document.status !== "active") throw notFoundAdminError()
...
function createGetResourceDocumentRoute({
  ...
  const handler: AdminRouteHandler<typeof routeConfig> = async (context) => {
    ...
    if (result === null) throw notFoundAdminError()
    const document = adminResourceDocumentDtoSchema.parse(result)
...
function createSaveResourceDocumentRoute({
  ...
  const handler: AdminRouteHandler<typeof routeConfig> = async (context) => {
    ...
    if (result.kind === "conflict") {
      const document = adminResourceDocumentDtoSchema.parse(result.document)
      return context.json(document, 412, {
        ETag: toIntegerEtag(document.version),
      })
    }
  ...
  const handler: AdminRouteHandler<typeof routeConfig> = async (context) => {
    ...
    return context.json(
      adminImportResourceDocumentResultDtoSchema.parse({
        ...
        mutation: { node: result.value.node },
      }),
      200
    )
  }
