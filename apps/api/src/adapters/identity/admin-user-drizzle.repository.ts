...
import type {
  ...
  DeleteAdminUserPersistenceResult,
  UpdateAdminUserStatusInput,
  UpdateAdminUserStatusPersistenceResult,
...
): AdminUserMutationRepository & AdminUserReader {
  return {
    deleteUser(input) {
...
type AdminUserSnapshot = AdminUserListItemDto
...
function updateUserStatus(
  ...
  const user = readUser(db, input)

  return user === null ? { kind: "not-found" } : { kind: "ok", value: user }
}
