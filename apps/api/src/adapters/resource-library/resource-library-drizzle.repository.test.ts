...
describe("통합 API 관리자 자료실 SQLite adapter", () => {
  it("3단계 트리, 조건부 저장, FTS, 재귀 휴지통과 자산 삭제 키를 한 모델로 처리한다", async () => {
    ...
    try {
      ...
      client.db
        ...
        .run()
      const resources = createResourceLibraryFixture(client.db)
...
function createResourceLibraryFixture(database: WritingAppDatabase) {
  return {
