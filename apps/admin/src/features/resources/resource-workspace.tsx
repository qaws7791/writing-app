...
import { resourceLibraryChangedEvent } from "@/features/resources/resource-library-model"
            ...
            {(["active", "trash"] as const).map((value) => (
              <Button
                ...
                onClick={() => {
                  ...
                  setSearch(null)
                  void api.getResourceTree(value).then((result) => {
                    if (result.status === "ok") setNodes(result.value.nodes)
