import type { DraftContent, HomeSnapshot } from "@/domain/draft"
...
export const fixturePrompts: SeedPrompt[] = [
  {
    description:
...
function toPromptDetail(prompt: SeedPrompt): PromptDetail {
  return {
...
function toPromptSummary(prompt: SeedPrompt): PromptSummary {
  return {
    id: prompt.id,
    level: prompt.level,
    saved: prompt.saved,
...
const fixtureDraftContent: DraftContent = {
  content: [
    {
      content: [
        {
          text: "첫 문장을 시작한 뒤에는 생각보다 다음 문장이 쉽게 따라오기도 합니다.",
          type: "text",
        },
      ],
      type: "paragraph",
...
export function createFixtureHomeSnapshot(): HomeSnapshot {
  const savedPrompts = fixturePrompts
    .filter((prompt) => [1, 6].includes(prompt.id))
    .map((prompt) => ({
      ...toPromptSummary(prompt),
      saved: true,
    }))

  const metrics = getDraftMetrics(fixtureDraftContent)
  const baseDraft = {
    characterCount: metrics.characterCount,
    content: fixtureDraftContent,
    createdAt: "2026-03-20T01:00:00.000Z",
    id: 101,
    lastSavedAt: "2026-03-20T09:10:00.000Z",
  ...
  return {
    recentDrafts: [baseDraft],
    resumeDraft: baseDraft,
    savedPrompts,
    todayPrompts: fixturePrompts.slice(0, 2).map(toPromptSummary),
  }
}

export function listFixturePromptSummaries(): PromptSummary[] {
  return fixturePrompts.map(toPromptSummary)
}

export function findFixturePrompt(promptId: number): PromptDetail | null {
  const prompt = fixturePrompts.find((item) => item.id === promptId)
  if (!prompt) {
    return null
  }
...
export function createFixtureDraftContent(): DraftContent {
  return structuredClone(createEmptyDraftContent())
}
