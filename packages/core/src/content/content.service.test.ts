...
async function getExistingLesson() {
  const result = await repository.findLesson(lessonId("sentence-structure-01"))
  if (result.status !== "ok") {
    throw new Error("Expected fixture lesson to exist.")
  }
  ...
  it("rejects lesson steps with non-contiguous order", async () => {
    const service = createContentService({
      repository: {
        ...repository,
        async findLesson() {
          const lesson = await getExistingLesson()
    ...
    const service = createContentService({
      repository: {
        ...
        async findLesson() {
          ...
          return {
            status: "ok",
            value: { ...lesson, steps: lesson.steps.slice(0, 1) },
          }
        },
      },
    })
    ...
    const service = createContentService({
      repository: {
        ...
        async findLesson() {
          ...
          const completeStep = lesson.steps[1]
