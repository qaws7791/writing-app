...
describe("makeGetVersionUseCase", () => {
  it("특정 버전의 상세 정보를 반환한다", async () => {
    ...
    const result = await getVersion(userId, draftId, 10)
  ...
  it("존재하지 않는 버전이면 NOT_FOUND를 반환한다", async () => {
    ...
    const result = await getVersion(userId, draftId, 99)
  ...
  it("존재하지 않는 문서이면 NOT_FOUND를 반환한다", async () => {
    ...
    const result = await getVersion(userId, toDraftId(999), 10)
  ...
  it("다른 사용자의 문서이면 NOT_FOUND를 반환한다", async () => {
    ...
    const result = await getVersion(toUserId("other-user"), draftId, 10)
