...
} from "#core/modules/learning/application/learner-read-projection"
...
describe("learner read page projection", () => {
  it("course page의 동률 row 순서와 다음 cursor primary를 보존한다", () => {
    ...
    expect(
      projectLearnerCoursePage({ limit: 1, rows, sort: "recommended" })
    ).toMatchObject({
      items: [{ id: "course-a" }],
    ...
    ).toEqual({ courseId: "course-a", primary: "같은 제목" })
  })
