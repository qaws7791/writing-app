...
function FilterToolbar({ className, ...props }: React.ComponentProps<"form">) {
  return (
    <form
      ...
      className={cn(
        "mb-4 grid grid-cols-[minmax(220px,1fr)_repeat(3,minmax(140px,auto))_auto_auto] items-end gap-3 rounded-panel border border-border-subtle bg-bg-surface p-4 max-lg:grid-cols-1",
        className
      )}
      {...props}
    />
...
function FilterToolbarLabel({
  ...
  return (
    <span
      ...
      className={cn("text-label-sm font-black text-fg-muted", className)}
      {...props}
    />
  )
}
