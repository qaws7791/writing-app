...
export type LessonStepRendererProps = {
  ...
  readonly onAnswerChange?: (
    _change: LessonAnswerChange
  ) => Promise<void> | void
  readonly onAnswerPayloadChange?: (_change: LessonAnswerPayloadChange) => void
}
    ...
    case "WRITE": {
      ...
      return (
        <WriteAnswer
          ...
          goal={step.goal}
          guide={guide}
          initialText={readLessonDraftText(handlers.draftNamespace, step.id)}
          max={step.max}
          min={step.min || 20}
          onChange={(text) => {
            writeLessonDraftText(handlers.draftNamespace, step.id, text)
            emitAnswer(handlers, step.id, {
              text,
              type: "WRITE",
            })
...
function assertNever(value: never): never {
  throw new Error(`지원하지 않는 레슨 스텝입니다: ${JSON.stringify(value)}`)
}
