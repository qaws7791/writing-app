...
function TextAnnotation({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex w-full flex-col gap-5 lg:flex-row lg:gap-6",
        className
      )}
      {...props}
    />
...
function TextAnnotationDocument({
  className,
  ...props
  ...
  "rounded-sm px-0.5 underline decoration-2 underline-offset-[0.2em] transition-colors duration-150",
  {
    variants: {
      kind: {
        ...
        spacing:
          "decoration-amber-600/50 bg-amber-500/10 dark:decoration-amber-400/50",
        agreement:
          "decoration-sky-600/50 bg-sky-500/10 dark:decoration-sky-400/50",
        logic:
          "decoration-orange-600/50 bg-orange-500/10 dark:decoration-orange-400/50",
        expression:
          "decoration-emerald-600/50 bg-emerald-500/10 dark:decoration-emerald-400/50",
      },
      state: {
        open: "opacity-100",
  ...
  return (
    ...
    >
      ...
      <span className="text-muted-foreground">
        {ANNOTATION_STATE_LABELS[state]}
      </span>
    </div>
  )
}

function TextAnnotationItemBody({
  className,
  ...props
...
function TextAnnotationItemActions({
  className,
  ...props
}: React.ComponentProps<"div">) {
  return (
