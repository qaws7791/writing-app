...
function RevisionHistoryHeader({
  className,
  ...props
  ...
  "grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl px-2.5 py-2.5",
  {
    variants: {
      state: {
        ...
        revision:
          "bg-foreground/[0.03] ring-1 ring-foreground/10 dark:bg-foreground/[0.05]",
        final:
          "bg-foreground/[0.04] ring-1 ring-foreground/10 dark:bg-foreground/[0.06]",
      },
    },
    defaultVariants: {
      state: "draft",
...
function RevisionEntryTitle({
  ...
  return (
    <div
      ...
      className={cn(
        "min-w-0 truncate text-sm font-medium tracking-[-0.01em]",
        className
      )}
      {...props}
    />
...
function RevisionHistoryCompare({
  className,
  ...props
}: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="revision-history-compare"
