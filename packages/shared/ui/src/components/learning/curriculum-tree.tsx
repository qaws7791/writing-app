...
function CurriculumTreeSummary({
  className,
  ...props
  ...
  "group/node flex w-full min-w-0 items-center gap-x-1.5 rounded-xl px-1.5 py-1 outline-none",
  {
    variants: {
      state: {
        ...
        locked: "opacity-70",
        published:
          "bg-foreground/[0.03] hover:bg-foreground/[0.05] dark:bg-foreground/[0.05]",
      },
      selected: {
...
function isCurriculumNodeChildren(
...
function CurriculumNodeLabel({
  ...
  return (
    <div
      ...
      data-level={level}
      className={cn(
        "min-w-0 flex-1 truncate text-sm font-medium tracking-[-0.01em]",
        className
      )}
      {...props}
    >
...
function CurriculumNodeActions({
  className,
  ...props
}: React.ComponentProps<"div">) {
  return (
...
function CurriculumNodeDropIndicator({
  className,
  ...props
}: React.ComponentProps<"div">) {
  return (
    <div
      ...
      aria-hidden
      className={cn(
        "pointer-events-none mx-2 h-0.5 rounded-full bg-foreground/40",
        className
      )}
      {...props}
    />
