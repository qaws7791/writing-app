...
function createUserColumns(
  ...
  return [
    ...
    },
    {
      ...
      header: "코호트",
      cell: ({ row }) => (
        <span className="text-muted-foreground">{row.original.cohort}</span>
      ),
    },
    {
      accessorKey: "status",
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="min-w-44">
              <DropdownMenuItem onClick={() => onDetailUser(user)}>
                기록 보기
              </DropdownMenuItem>
              {user.status === "invited" ? (
...
function UsersDataTable({ data }: { data: UserRow[] }) {
  ...
  const columns = React.useMemo(
    () => createUserColumns(setDetailUser),
    [setDetailUser]
  )
            ...
            <div className="overflow-y-auto px-6 py-5">
              <LearnerRecord>
                ...
                <LearnerRecordSection>
                  ...
                  <LearnerRecordPath>
                    {detailUser.progressLabel}
                  </LearnerRecordPath>
                </LearnerRecordSection>
                <LearnerRecordSection>
                  <LearnerRecordSectionTitle>숙련</LearnerRecordSectionTitle>
                  <LearnerRecordMastery>
                    {detailUser.masteryLabel}
                  </LearnerRecordMastery>
                  <LearnerRecordAttempts>
                    {detailUser.attemptsLabel}
                  </LearnerRecordAttempts>
                </LearnerRecordSection>
                <LearnerRecordSection>
                  <LearnerRecordSectionTitle>지원</LearnerRecordSectionTitle>
                  <LearnerRecordSupport>
                    {detailUser.supportLabel}
                  </LearnerRecordSupport>
                </LearnerRecordSection>
              </LearnerRecord>
              <p className="mt-4 text-xs leading-5 text-muted-foreground">
                전체 계정·학습 기록·지원 화면은 user-admin 상세 블록에서 확인할
                수 있습니다.
              </p>
            </div>
          ) : null}
