...
export function validateCurriculum(units: UnitNode[]): ValidationIssue[] {
  ...
  for (const [unitIndex, unit] of units.entries()) {
    ...
    for (const [lessonIndex, lesson] of unit.lessons.entries()) {
      ...
      const stepErrorCount = stepIssues.filter(
...
export function countIssuesForLesson(
...
export function countIssuesForStep(
  issues: ValidationIssue[],
  lessonId: string,
  stepId: string
) {
  ...
  switch (action.type) {
    case "hydrate":
      return {
        ...
        expanded: Object.fromEntries(
          action.units.map((unit) => [unit.id, true])
        ),
        undo: null,
      }
    case "select":
    ...
    case "lesson/add": {
      ...
      const units = state.units.map((unit) =>
        unit.id === action.unitId
          ? { ...unit, lessons: [...unit.lessons, lesson] }
          : unit
      )
    ...
    case "step/add": {
      ...
      return withUndo(
        state,
        units,
        `${STEP_TYPE_LABELS[action.stepType]} 스텝을 추가했습니다.`,
        {
          selection: {
            kind: "step",
            ...
            steps: lesson.steps.map((step) => {
              ...
              if (action.patch.type && action.patch.type !== step.type) {
                ...
                return {
                  ...
                  prompt: step.prompt,
                  title:
                    action.patch.title ?? STEP_TYPE_LABELS[action.patch.type],
                }
              }
    ...
    case "step/remove": {
      const hit = findStep(state.units, action.lessonId, action.stepId)
      if (!hit) return state
      const removedIndex = hit.lesson.steps.findIndex(
        (step) => step.id === action.stepId
      )
      const units = state.units.map((unit) => ({
        ...unit,
    ...
    <Field>
      ...
      <div className="flex flex-col gap-2">
        ...
        <Button
          ...
          className="w-fit"
          onClick={() =>
            onChange([...options, { id: nextId("opt"), label: "" }])
          }
        >
          <HugeiconsIcon
            icon={PlusSignIcon}
    ...
    <Field>
      ...
      <div className="flex flex-col gap-2">
        ...
        <Button
          ...
          className="w-fit"
          onClick={() =>
            onChange([...items, { id: nextId("item"), label: "" }])
          }
        >
          <HugeiconsIcon
            icon={PlusSignIcon}
    ...
    <FieldGroup className="gap-4">
      ...
      {step.type === "FILL_BLANK" ? (
        <>
          ...
          />
          <Field>
            <FieldLabel htmlFor={`step-answers-${step.id}`}>
              정답 토큰 (쉼표로 구분)
            </FieldLabel>
            <Input
...
function SortableUnitRow({
  ...
  onCancelRename: () => void
  onMenu: (
    action: "up" | "down" | "duplicate" | "remove" | "add-lesson" | "rename"
  ) => void
      ...
      >
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-44">
            <DropdownMenuItem onClick={() => onMenu("add-lesson")}>
              레슨 추가
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onMenu("rename")}>
              이름 바꾸기
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onMenu("duplicate")}>
              복제
            </DropdownMenuItem>
            <DropdownMenuSeparator />
        ...
        <DragDropProvider
          ...
          onDragEnd={(event) => {
            ...
            const data = source.data as
              | { kind?: string; id?: string; unitId?: string }
              | undefined
            if (data?.kind === "unit") {
              dispatch({
                type: "unit/move",
              ...
              return (
                <React.Fragment key={unit.id}>
                  <SortableUnitRow
                    ...
                    expanded={isExpanded}
                    renaming={
                      renaming?.kind === "unit" && renaming.id === unit.id
                    }
                    ...
                    onToggle={() =>
                      dispatch({ type: "toggle-expand", id: unit.id })
                    }
                    onRename={(title) =>
                      dispatch({
                        type: "unit/update",
...
function UnitEditor({
  ...
  return (
    <section
      data-slot="curriculum-builder-unit-editor"
...
function StepEditSheet({
  ...
  const stepIssues = step
    ? issues.filter(
        (issue) =>
          issue.selection.kind === "step" && issue.selection.stepId === step.id
      )
    : []
  const errorCount = stepIssues.filter(
    (issue) => issue.severity === "error"
  ).length
  const warningCount = stepIssues.filter(
...
export function CurriculumBuilder({
  ...
  const lastNotifiedSelection = React.useRef<
    CurriculumSelection | null | undefined
    ...
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (
        ...
        if (current.kind === "unit") {
          ...
          if (!hit) return
          const from = hit.unit.lessons.findIndex(
          ...
          if (!hit) return
          const from = hit.lesson.steps.findIndex(
  ...
  const selectedUnit =
    state.selection?.kind === "unit"
      ? findUnit(state.units, state.selection.id)
      : undefined
  const lessonId =
  ...
  const selectedStepId =
    state.selection?.kind === "step" ? state.selection.stepId : undefined
  const editingStep =
    state.selection?.kind === "step"
      ? (findStep(state.units, state.selection.lessonId, state.selection.stepId)
          ?.step ?? null)
      : null
  const editingPathLabel = React.useMemo(() => {
    ...
    const unitIndex = state.units.findIndex(
      (unit) => unit.id === lessonHit.unit.id
    )
    const lessonIndex = lessonHit.unit.lessons.findIndex(
      (lesson) => lesson.id === lessonHit.lesson.id
    )
    const stepIndex = lessonHit.lesson.steps.findIndex(
      (step) => step.id === stepSelection.stepId
    )
    ...
    >
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[minmax(14rem,18rem)_minmax(0,1fr)] grid-rows-[minmax(0,1fr)] gap-3">
        ...
        >
          ...
          {state.units.length > 0 && !state.selection ? (
            <Empty variant="compact" className="min-h-48">
              <EmptyHeader>
                ...
                <EmptyDescription>
                  왼쪽에서 단원을 고르면 정보를 수정하고, 레슨을 고르면 스텝을
                  편집합니다.
                </EmptyDescription>
              </EmptyHeader>
            </Empty>
          ...
          {selectedUnit ? (
            <UnitEditor unit={selectedUnit} dispatch={dispatch} />
          ) : null}
          ...
          {lessonHit ? (
            <LessonEditor
              ...
              unitIndex={state.units.findIndex(
                (unit) => unit.id === lessonHit.unit.id
              )}
