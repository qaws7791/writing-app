...
export type AutosaveDraftUseCaseDependencies = {
 ...
 */
export function makeAutosaveDraftUseCase(
  dependencies: AutosaveDraftUseCaseDependencies
) {
  return async (
    userId: UserId,
    draftId: DraftId,
    ...
    const existing = await dependencies.draftRepository.getById(userId, draftId)
    ...
    const persisted = await dependencies.draftRepository.replace(
      userId,
      draftId,
      {
...
export async function autosaveDraftUseCase(
  ...
  return makeAutosaveDraftUseCase({
    draftRepository,
    getNow,
  })(userId, draftId, input)
}
