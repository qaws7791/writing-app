...
- 저장소는 SQLite를 사용하고, Tiptap 문서는 HTML 없이 SQLite JSONB BLOB으로 저장한다.
...
- `drafts.body_jsonb`에는 Tiptap JSON 문서를 저장한다.
