*중요: 이 문서와 관련된 모든 구현 작업을 할 때는  `vendor/heroui`의 실제 소스 코드를 참조하여 heroui v3의 패턴과 구조를 최대한 정확히 재현하는 것을 목표로 해야 합니다. 이 문서는 아키텍처와 설계 원칙을 설명하는 거시적 계획이므로, 세부 구현에서는 반드시 heroui v3의 실제 코드에서 추출한 규칙과 패턴을 엄격히 따라야 합니다.`vender` 디렉토리는 git으로 커밋하지 마세요*
...
두 번째 목표는 **개발자 경험(DX) 최적화**입니다. heroui v3의 2-패키지 분리 구조는 프레임워크 무관성이라는 범용성에서 비롯되지만, 우리는 React 전용이므로 styles와 react를 단일 패키지로 통합하여 `import { Button } from "@workspace/ui"` 한 줄이면 스타일과 행위가 모두 포함되는 구조를 지향합니다.
...
- **Import 경로 단순화**: `import { Button, buttonVariants } from "@workspace/ui"`
...
┌─────────────────────────────────────────────────┐
│  소비자 앱 (apps/web)                            │
│  @import "@workspace/ui/styles" in global CSS        │
...
@import "tailwindcss";
@import "@workspace/ui/styles";          /* 토큰 + 테마 + 컴포넌트 CSS */
...
`@workspace/ui/styles`는 `package.json`의 `exports` 필드를 통해 `./src/styles/index.css`를 가리킵니다.
...
```ts
import { tv, buttonVariants } from "@workspace/ui";
...
import { Button, Modal, Select } from "@workspace/ui";
...
- 소비자 앱에서 `@import "@workspace/ui/styles"` 동작 확인
...
내부 패키지이므로 npm 퍼블리시가 아닌 **모노레포 내 workspace 참조**가 기본입니다. 소비자 앱은 `"@workspace/ui": "workspace:*"`로 의존성을 선언합니다.
