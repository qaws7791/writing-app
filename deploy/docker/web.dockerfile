...
ARG API_BASE_URL
ARG WEB_ORIGIN
ARG CONTENT_ASSET_IMAGE_ALLOWED_ORIGINS
ARG CSP_REPORT_ONLY=false

ENV NODE_ENV=production \
    API_BASE_URL=${API_BASE_URL} \
    WEB_ORIGIN=${WEB_ORIGIN} \
    CONTENT_ASSET_IMAGE_ALLOWED_ORIGINS=${CONTENT_ASSET_IMAGE_ALLOWED_ORIGINS} \
    CSP_REPORT_ONLY=${CSP_REPORT_ONLY}

RUN test -n "$API_BASE_URL" \
    && test -n "$WEB_ORIGIN" \
    && test -n "$CONTENT_ASSET_IMAGE_ALLOWED_ORIGINS"
