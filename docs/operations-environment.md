...
- 플랫폼과 어드민은 같은 SQLite 파일을 사용할 수 있지만 인증 테이블과 쿠키는 분리한다.
...
로컬 예시는 각 앱의 `.env.example`을 기준으로 만든다.
...
| `DATABASE_URL`         | `file:/var/lib/writing-app/app.sqlite` | 어드민 API와 같은 파일을 사용할 수 있다. |
...
| `DATABASE_URL`             | `file:/var/lib/writing-app/app.sqlite` | 플랫폼 API와 같은 파일을 사용할 수 있다.       |
...
같은 이메일로 다시 실행하면 중복 생성하지 않는다. `ADMIN_SEED_PASSWORD`는 시드 완료 후 운영 비밀값 저장소에서 제거한다.
...
- `DATABASE_URL`이 두 API에서 같은 SQLite 파일을 가리키는 경우, 파일 권한과 백업 정책을 API 프로세스 계정 기준으로 확인한다.
