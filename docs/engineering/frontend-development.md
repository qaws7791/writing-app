...
- 점검과 마치기는 서버 결과를 표시한다. client가 점검 가능 여부나 마치기 가능 여부를 다시 계산하지 않는다.

## 어드민 경험

- 어드민은 학습자 인증과 분리된 관리자 로그인을 사용한다.
- 콘텐츠 관리는 카테고리·상태 필터, 페이지 크기, 페이지 이동, 새 코스 생성, 코스 보관을 제공한다.
- 쓰기 과제는 검색·도메인·상태 필터, 새 과제 생성, 초안 저장과 발행을 제공한다. 이미 시작한 글의 고정 발행본은 바꾸지 않는다.
- 코스 편집은 유닛, 레슨 배치, 레슨 추가/보관, 순서 변경, 표준 8개 스텝 타입별 편집 폼을 제공한다.
