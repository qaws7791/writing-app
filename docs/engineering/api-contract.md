...
- 작업 상태: 2026-07-17 MTA-4 HTTP 보안 계약 기준선 반영
- 기준일: 2026-07-17
- 기준 파일:
...
MTA-4는 production 정책을 바꾸지 않고 공통 Hono middleware와 학습자 API·관리자 host sub-app의 최종 HTTP 응답을 함께 고정한다. 공통 fixture는 같은 입력과 관찰 항목을 사용하되 API 표면별 기대 결과를 명시적으로 분리한다.
...
관리자 브라우저의 기준 URL은 `NEXT_PUBLIC_ADMIN_API_BASE_URL`, 서버 측 호출의 기준 URL은 `ADMIN_API_BASE_URL`이 가리키는 관리자 API origin이다. MTA-40 저장소 구성은 관리자 host를 `apps/api`의 admin host sub-app으로 보내며, 관리자 SSR의 내부 기준 URL은 `http://admin-api-unified:4000`이다. 브라우저의 공개 origin은 그대로 관리자 API host를 사용한다. 이 사실은 저장소 구성에 대한 검증 결과이며, 실제 production 적용과 관찰 완료를 뜻하지 않는다.
...
- OpenAPI는 `apps/api` admin Host sub-app에 실제 등록된 여섯 capability route와 foundation route에서 생성한다.
