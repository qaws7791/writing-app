...
OpenAPI와 Orval client는 runtime과 같은 route source에서 build task로 결정적으로 재생성하며 산출물을 Git에 보존하지 않는다. 생성 순서와 schema 검증은 Turbo dependency와 계약 테스트로 확인한다. migration SQL은 application migration source를 Git에 보존한다. Storybook 정적 산출물은 source가 아니며 필요할 때 manifest의 build task로 재생성한다. 현재 source는 API route·migration directory와 Storybook manifest를 직접 확인한다.
