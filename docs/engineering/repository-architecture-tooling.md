...
기존 flat package를 전환하는 동안 `dependency-cruiser.config.mjs`의 `legacy-*` 규칙이 같은 정책을 적용한다. identity와 content 전환은 완료됐고 `legacy-core-*`의 나머지 범위는 P6~P9와 P15가 소유한다. `legacy-ui-*`는 P12, `legacy-frontends-*`는 P12, 나머지 API 경계 규칙은 P6~P10에서 대상 package 규칙으로 흡수한 뒤 P15에서 제거한다. 디렉터리 전체를 통과시키는 임시 allowlist는 허용하지 않으며 새 예외에는 정확한 edge, owner, 제거 단계와 만료 조건이 필요하다.

P5 이후 아직 전환되지 않은 AI feedback, learning과 operations repository의 content schema 직접 조회만 파일 단위로 허용한다. 각각 P6, P7, P9에서 공개 application·query port로 치환하면 같은 단계에서 예외를 제거한다. DB infra에서 content 정책으로 향하는 예외는 두지 않고, legacy curriculum 정규화는 API가 양쪽 공개 경계를 조립해 주입한다.
