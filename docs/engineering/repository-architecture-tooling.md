...
기존 flat package를 전환하는 동안 `dependency-cruiser.config.mjs`의 `legacy-*` 규칙이 같은 정책을 적용한다. identity, content, ai-feedback, learning과 resource-library 전환은 완료됐고 `legacy-core-*`의 나머지 범위는 P9와 P15가 소유한다. `legacy-ui-*`와 `legacy-frontends-*`는 P12, 나머지 API 경계 규칙은 P9~P10에서 대상 package 규칙으로 흡수한 뒤 P15에서 제거한다. 디렉터리 전체를 통과시키는 임시 allowlist는 허용하지 않으며 새 예외에는 정확한 edge, owner, 제거 단계와 만료 조건이 필요하다.
