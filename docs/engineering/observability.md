...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------- | --------------------------------- |
| `request.completed` | requestId, route template인 `path`, method, status, durationMs, audience, actorType/actorId, oauthClientId, outcome/errorClass | `application-30d`                        | 결과와 무관하게 `info`            |
| `security.audit`    | requestId, action, template 기반 target, actorType/actorId, oauthClientId, outcome, reasonCode, 제한된 clientIp/userAgent      | `security-90d`                           | 성공은 `info`, 거절·실패는 `warn` |
...
- 관리자 MCP 요청은 `admin-mcp` audience, 검증된 owner 관리자 ID와 introspection에서 받은 OAuth client ID를 기록한다.
- 관리자 MCP 로그는 access token, Authorization header, tool 입력과 tool 출력을 기록하지 않는다.
- 관리자 MCP 변경 감사는 실행 ID, nullable 승인 ID, 입력 digest와 검증된 OAuth client ID를 DB 원장에 기록한다.
...
- 감사 화면은 MCP 변경의 실행 ID와 OAuth client ID를 표시한다.
