...
- 공통 logger와 request·security event schema는 observability infra가 소유한다. API composition은 인증·인가 결과, module event 발행 실패와 provider observer를 이 경계에 연결하고, 각 module은 원문 payload가 아닌 typed failure event만 공개한다.
