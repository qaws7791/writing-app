...
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | --------------------------------- |
| `request.completed` | requestId, route template인 `path`, method, status, durationMs, audience, actorType/actorId, mcpCredentialId, outcome/errorClass | `application-30d`                        | 결과와 무관하게 `info`            |
| `security.audit`    | requestId, action, template 기반 target, actorType/actorId, mcpCredentialId, outcome, reasonCode, 제한된 clientIp/userAgent      | `security-90d`                           | 성공은 `info`, 거절·실패는 `warn` |
...
- 모든 관리자 MCP HTTP 요청은 `admin-mcp` audience의 `request.completed` event를 남긴다.
- 인증된 관리자 MCP 요청은 검증된 owner 관리자 ID와 MCP credential ID를 기록한다.
- 인증·인가 거절은 `security.audit` event를 추가로 남긴다.
- 관리자 MCP 로그는 raw token, token digest, Authorization header, tool 입력과 tool 출력을 기록하지 않는다.
- 모든 관리자 MCP 요청을 operations DB audit에 복제하지 않는다.
- 정책상 영속 감사 대상인 변경과 민감 작업만 operations DB audit에 기록한다.
- 관리자 MCP 변경 감사는 실행 ID, nullable 승인 ID, 입력 digest와 검증된 MCP credential ID를 DB 원장에 기록한다.
...
- 2단계 자동 실행의 각 호출은 감사 row를 남긴다. 재시도 row는 같은 실행 ID와 영수증 재생 결과를 공유한다.
- 3단계 재시도는 승인 ID에서 결정한 같은 감사 row를 재사용한다.
- 감사 화면은 MCP 변경의 실행 ID와 MCP credential ID를 표시한다.
- 감사 화면은 승인 ID가 있는 3단계 변경에만 승인 요청 링크를 표시한다.
