...
- 완료
- 2026-07-14 재검증: 통과. Codex 인앱 브라우저에서 로컬 3001/4001 개발 서버의 로그인 화면과 잘못된 인증 정보 오류를 확인했고, 전용 SQLite·서버를 사용하는 Playwright 통합 브라우저 시나리오 3건을 다시 실행해 모두 통과했다.
...
| 검증 항목                                             | 결과                                                                                                                                            |
| ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `ENABLE_TEST_AUTH=true bunx bun@1.3.10 run dev:admin` | 통과. 관리자 웹 3001과 API 4001이 기동했다.                                                                                                     |
| Codex 인앱 브라우저 `http://localhost:3001/login`     | 통과. 로그인 입력과 잘못된 인증 정보 오류를 확인했다. 로컬 DB의 기존 seed 암호와 환경 파일이 불일치해 정상 로그인은 격리 E2E 환경에서 검증했다. |
| `ENABLE_TEST_AUTH=true bunx bun@1.3.10 run test:e2e`  | 통과. UI 시각 계약, 학습자 테스트 로그인 흐름, 어드민 owner/operator 권한·MFA·콘텐츠·사용자·자료실 시나리오 3건이 통과했다.                     |
| 종료 자원 확인                                        | 통과. 3001, 4001, 3100, 3101, 4100, 4101 포트와 `apps/admin/.next/dev/lock`이 모두 해제됐다.                                                    |
...
### 2026-07-14 MFA 제거 후 재검증

| 검증 항목                                            | 결과                                                                                                              |
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `ENABLE_TEST_AUTH=true bunx bun@1.3.10 run test:e2e` | 통과. UI 시각 계약, 학습자 테스트 로그인, 비밀번호만 사용하는 어드민 owner/operator 권한 시나리오 3건이 통과했다. |
| 어드민 owner 로그인                                  | 통과. TOTP 등록·입력·복구 코드 화면 없이 이메일·비밀번호 로그인 후 권한 범위로 바로 이동한다.                     |
| 어드민 operator 인가                                 | 통과. operator는 로그인할 수 있고 owner 전용 변경 작업은 서버 경계에서 `403 forbidden`을 받는다.                  |
