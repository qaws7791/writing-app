...
학습자·관리자 request/response DTO와 Zod schema는 `packages/contracts`가 원본으로 소유한다. 브라우저 앱은 `@workspace/contracts/*`만 import하고, `packages/core`는 기존 public API 호환성을 위해 contracts를 re-export한다.
