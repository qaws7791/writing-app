...
학습자 웹 source는 `app`, `features`, `entities`, `shared`, `server`의 단방향 계층을 사용한다. `app` route는 Zod 입력 파싱, 인증·redirect, feature-local DAL 호출과 화면 조립만 수행한다. 서버 조회는 Server Component가 소유하고, 브라우저 갱신이 필요한 feature만 자기 `api` 포트와 최소 Client Component를 사용한다. architecture test가 다른 feature 내부 import, shared/server 역방향 import, model의 React·I/O 의존, client→server와 UI→DAL import를 차단한다.
...
매칭 스텝의 presentation choice id, deterministic shuffle, pending·selection map과 stable item-ID 답안 변환은 `apps/web/src/features/lesson-session/model`이 소유한다. `packages/ui`의 `MatchAnswer`는 controlled choice·connection을 표시하고 button 접근성과 연결선 DOM 측정만 담당한다. `packages/contracts`는 학습 DTO schema만 노출하고, core는 웹 UI 상호작용 모델을 재수출하지 않는다.
