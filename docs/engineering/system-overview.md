...
어드민 API는 현재 `apps/admin-api`에서 `@workspace/db`를 직접 조립해 `@workspace/core/admin/admin-drizzle.repository`의 `createDrizzleAdminRepository(database.db)`에 넘긴다. 어드민 서비스 구현 자체는 `packages/core/admin`에 둔다. 서비스 생성자는 dashboard, analytics, course, user, settings, content reset 기능별 port를 명시적으로 받아 단일 mega repository 의존이 다시 숨지 않게 한다.
