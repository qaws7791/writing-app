...
관리자 자료실과 관리자 AI 채팅은 어드민 API 경계에 속한다. 자료실 구조 명령과 조회·검색·가져오기·내보내기는 REST를 사용하고, 본문 공동 편집과 구조 변경 알림은 같은 `apps/admin-api` 프로세스의 WebSocket을 사용한다. AI 채팅은 `admin_ai_chat_conversations`, `admin_ai_chat_messages`에 대화 내역을 저장하고, `apps/admin-api`에 내장한 Mastra `admin-content-agent`를 Hono route에서 호출해 `chunk`, `done`, `error` SSE 이벤트로 응답한다. Mastra Memory는 사용하지 않고 저장된 메시지를 프롬프트 컨텍스트로 구성한다.

`packages/resource-document`는 브라우저·headless 서버 공용 Lexical 0.46.0 node, 정규 GFM AST import/export와 검증, Yjs snapshot 적용·투영을 제공한다. `apps/admin-api/src/collaboration/y-websocket-bun-adapter.ts`는 공식 Yjs 13 `sync`·`awareness` 공개 protocol API를 Bun `ServerWebSocket`에 연결하고 socket별 전송 실패를 격리한다. 운영 `/resources/collaboration/{documentId}`와 `/resources/events` upgrade는 관리자 session cookie와 정확한 origin을 검증한다. 문서 room은 최대 20명, debounce projection, 종료·마지막 연결·내보내기·휴지통 이동 flush를 제공하며 실패는 해당 room에만 격리한다. `/resources/events`는 작업 공간 수명의 지속 연결에서 활성 문서 구독과 heartbeat를 받고, 문서별 version·무효화 사건을 구독자에게만 전달하며 관리자 ID 기준 활성 편집자 수를 계산한다. 서버의 HTTP 동기화 Module은 멱등 transaction 저장, 최근 update pull과 snapshot fallback을 제공하지만 본문 클라이언트 Adapter는 아직 문서별 공동 편집 WebSocket을 사용한다. `apps/admin` 자료실은 reui 기반 지연 로딩 트리와 Lexical WYSIWYG 편집기, 슬래시 메뉴, 블록 이동, 선택 서식 도구, 재연결 병합과 동기화 상태를 제공한다.
