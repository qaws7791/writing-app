...
`packages/resource-document`는 브라우저·headless 서버 공용 Lexical 0.46.0 node, 정규 GFM AST import/export와 검증, Yjs snapshot 투영을 제공한다. `apps/admin-api/src/collaboration/y-websocket-bun-adapter.ts`는 공식 Yjs 13 `sync`·`awareness` 공개 protocol API를 Bun `ServerWebSocket`에 연결하고 socket별 전송 실패를 격리한다. 운영 `/resources/collaboration/{documentId}`와 `/resources/events` upgrade는 관리자 session cookie와 정확한 origin을 검증한다. 문서 room은 최대 20명, debounce projection, 종료·마지막 연결·내보내기·휴지통 이동 flush를 제공하며 실패는 해당 room에만 격리한다. `apps/admin` 자료실은 reui 기반 지연 로딩 트리와 Lexical WYSIWYG 편집기, 슬래시 메뉴, 블록 이동, 선택 서식 도구, 재연결 병합과 동기화 상태를 제공한다.
