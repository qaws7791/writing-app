...
- 학습자 데이터 삭제는 자기 table을 소유한 module이 실행한다([ADR-0027](./adr/ADR-0027-learner-data-purge-module-ports.md)). 각 module이 `LearnerDataPurgePort` 구현을 공개하고 조립 지점이 FK 의존 순서를 고정한 배열로 순회하며, 전체가 하나의 SQLite transaction에서 원자적으로 실행된다. 조립 지점 밖에서 다른 module의 table을 지우는 경로는 architecture 검사가 차단한다.
