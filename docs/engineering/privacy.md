...
| 답안·학습 상태 | 학습자가 작성한 레슨 답안·초안·독립 글과 서버가 생성한 진도·활동을 저장, 재개와 완료 표시 목적으로 사용한다.                                                                                                                                                        | 회원 유지 기간과 삭제 요청 후 5일.                                                                                                                                                | 사용자 정리 transaction이 초안·답안·글·쓰기 event·진도·활동을 물리 삭제한다. 콘텐츠 revision은 함께 삭제하지 않는다.                                                                                | AI 코칭을 요청한 레슨 답안만 아래 OpenAI 경계로 전달한다.                         |
...
| 관리자 감사    | opaque actor·target ID, action, request ID, outcome과 필요 시 검증된 IP를 개인정보 조회와 고위험 변경의 책임 추적에 사용한다. MCP 변경은 실행 ID, nullable 승인 ID, 입력 digest와 OAuth client ID를 추가한다. 이메일·이름·답안·prompt 원문은 payload에 넣지 않는다. | 개인정보 조회·콘텐츠 생성·초안 저장·발행·보관·복원 1년, 사용자 상태 변경·삭제 3년.                                                                                                | `retention_until` 경계의 bounded 일일 정리로 DB row를 물리 삭제한다.                                                                                                                                | 외부 log의 `audit.recorded`는 별도 class 정책을 따르며 DB 원장을 대체하지 않는다. |
...
- 승인 요청은 대상 제목 또는 opaque 식별자, 상태, OAuth client ID, 입력 digest와 시각만 저장한다.
- 자동 실행 영수증은 실행 ID, owner 관리자 ID, OAuth client ID, Tool, 멱등 키, 입력 digest와 최소 결과 metadata만 저장한다.
...
- 학습자 데이터 삭제는 자기 table을 소유한 module이 실행한다([ADR-0027](./adr/ADR-0027-learner-data-purge-module-ports.md)). 각 module이 `LearnerDataPurgePort` 구현을 공개하고 조립 지점이 FK 의존 순서를 고정한 배열로 순회하며, 전체가 하나의 SQLite transaction에서 원자적으로 실행된다. writing module은 글 원문과 쓰기 event를 함께 삭제한다. 조립 지점 밖에서 다른 module의 table을 지우는 경로는 architecture 검사가 차단한다.
