...
- `createApp()`은 route 등록, validation error hook, 404 handler, error handler를 고정 순서로 설치한다.
...
- 현재 `@hono/zod-openapi` 타입 추론 특성상 `c.req.valid()` 타입을 명확히 보존해야 하는 route는 route config를 `as const`로 먼저 선언하고 handler를 `RouteHandler<typeof routeConfig>`로 묶어 사용한다.
