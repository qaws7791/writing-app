...
- `TRUE_FALSE`: 질문, 판정할 진술, 참·거짓 정답, 해설을 제공한다. 정답이 boolean이므로 stable ID를 쓰지 않는 유일한 채점 타입이다.
- `SENTENCE_BUILD`: 질문, stable ID가 있는 어절 타일, 정답 ID 순서와 해설을 제공한다. 정답은 타일 전체가 아니라 부분집합일 수 있으므로 정답에 없는 타일은 방해 타일로 남는다.
- `ERROR_CORRECT`: 질문, stable ID가 있는 문장 구간과 교정안, 오류 구간 정답 ID 하나와 교정안 정답 ID 하나, 해설을 제공한다. 두 정답을 모두 맞혀야 정답으로 판정한다.

## 제외 스텝 타입

- `COPY`, `COMPLETION`, `ANNOTATE`, `SUMMARY`, `TRANSCRIBE`, `PARAGRAPH_ORGANIZE`는 현재 표준 스텝 타입으로 사용하지 않는다.
- 필사, 완료, 주석, 요약, 원문 전사, 문단 조직 활동이 필요해지면 새 스텝 타입 추가 절차에 따라 별도 제품 요구사항과 DTO 계약을 먼저 정의한다.
