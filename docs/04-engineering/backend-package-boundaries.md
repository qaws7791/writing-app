---
title: 백엔드 패키지 경계
description: apps/api, core, db, storage, ai 패키지의 책임과 금지 의존성을 정의합니다.
---
...
- 실제 저장소에는 아직 `packages/core`, `packages/db`, `packages/storage`, `packages/ai`가 존재하지 않습니다.
...
| `packages/core`    | 비즈니스 규칙, 계약 스키마, 상태 전이, use case, 포트       | Hono, DB 드라이버, 스토리지 SDK, AI SDK             |
