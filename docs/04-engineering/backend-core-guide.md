    ...
    modules/
      ...
      journeys/
        ...
        operations/
          start-session.ts
          complete-session.ts
          unlock-next-session.ts
        ports/
          journey-repository.ts
          session-progress-repository.ts
        use-cases/
          get-journey-use-case.ts
          enroll-journey-use-case.ts
          start-session-use-case.ts
          submit-step-use-case.ts
      ...
      ai-feedback/
        contracts/
          ai-feedback-contract.ts
        model/
          feedback.ts
        ports/
          ai-coaching-gateway.ts
        use-cases/
          generate-feedback-use-case.ts
          compare-revisions-use-case.ts
        fixtures/
          feedback-fixture.ts
      home/
        contracts/
        use-cases/
          get-home-use-case.ts
        fixtures/
  tests/
```
...
- `start-session` 연산은 이전 세션 완료 여부를 확인하고 새 진행 상태를 반환합니다.
- `generate-feedback-use-case`는 `AiCoachingGateway`, `WritingRepository` 같은 포트를 받아 조합합니다.
- 유효성 검사 실패, 중복 리소스, 정책 위반은 예외를 던지지 않고 실패 값으로 반환합니다.
- 영속화 전후 매핑은 코어가 아니라 `packages/database`에서 처리합니다.
