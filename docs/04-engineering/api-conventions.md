...
- `apps/web/foundation/api/schema.d.ts`: 서버 OpenAPI 스펙에서 `openapi-typescript`로 자동 생성한 타입입니다.
- `apps/web/foundation/api/client.ts`: `openapi-fetch` 기반 타입 세이프 HTTP 클라이언트입니다.
- 타입 생성 명령: `bun run api:generate` (API 서버 실행 필요)
