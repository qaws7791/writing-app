...
- `packages/core`, `packages/db`, `packages/storage`, `packages/ai`는 아직 생성되지 않았습니다.
...
| 백엔드 전체 테스트    | `turbo test --filter api --filter @workspace/core --filter @workspace/db --filter @workspace/storage --filter @workspace/ai` |
