...
- 백엔드는 `packages/core`, `packages/database`, `packages/ai` 구조로 구현 중입니다.
- 프론트엔드는 여정, 세션, 글쓰기, AI 피드백 화면 프로토타입 위주입니다.
...
- 앱/패키지 런타임 코드에서 `console.*`는 금지하고 구조화 로거를 사용합니다.
- 사용자 대상 CLI 출력이 필요한 `**/scripts/**`만 oxlint override로 `console.*`를 허용합니다.
...
- 조건부 Tailwind className은 `cn()`으로 조합하여 가독성을 높입니다.

## 컴포넌트 파일 네이밍 규칙

파일 이름은 `{도메인}-{역할}` 패턴을 따릅니다.

- **도메인**: 컴포넌트가 속한 기능 영역 (예: `writing`, `journey`, `session`, `prompt`, `auth`)
- **역할**: 컴포넌트가 수행하는 구체적인 역할 (예: `text-editor`, `editor-body`, `editor-header`, `step-card`)
...
features/session-flow/components/
├── session-step-card.tsx         # session 도메인 / step-card 역할
├── session-progress-bar.tsx      # session 도메인 / progress-bar 역할
└── session-feedback-panel.tsx    # session 도메인 / feedback-panel 역할
```
...
- 도입 예정인 `packages/database`, `packages/ai`는 각 인프라 책임만 수행합니다.
