...
- 앱/패키지 런타임 코드에서는 `console.*`를 사용하지 않고 구조화 로거를 사용합니다.
- `console.*` 예외는 사용자 대상 CLI 출력이 필요한 `**/scripts/**`에만 oxlint override로 허용합니다.
...
- `apps/api`, `apps/admin` 서버 코드는 `@workspace/logging`의 pino 기반 구조화 로거를 사용합니다.
- 로거 메타데이터에는 `service`, 요청/작업 식별자, 안전한 운영 메타데이터만 남깁니다.
- 파일 본문, 비밀 값, 토큰, 사용자 원문, 원본 업로드 파일명 같은 값은 로그에 남기지 않습니다.
- 운영상 불가피한 예외가 있으면 전역 해제 대신 `// oxlint-disable-next-line no-console`로 한 줄 단위에서만 허용합니다.
