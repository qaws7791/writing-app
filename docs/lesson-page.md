...
## 2026-06-15 시작 — 몰입형 레슨 레이아웃

- 레슨 화면에서 글로벌 네비게이션 헤더와 모바일 네비게이션을 제거한다.
- 시작 화면과 학습 스텝 화면 모두 상단 레슨 헤더와 하단 CTA 영역은 화면 안에 고정하고, 중앙 콘텐츠 영역만 스크롤되게 한다.
- 색상 체계는 현재 제품 테마를 유지하고, 이번 작업에서는 레이아웃 구조만 조정한다.

## 2026-06-15 완료 — 몰입형 레슨 레이아웃

- `/app/lesson` route를 일반 학습자 앱 route group과 분리해 `AppShell`의 글로벌 헤더와 모바일 네비게이션이 렌더링되지 않게 했다.
- 시작 화면과 학습 스텝 화면은 공통 `LessonShell`을 사용한다.
- 레슨 셸은 viewport 높이 안에서 `overflow-hidden`을 유지하고, 상단 진행 헤더와 하단 CTA 푸터는 `shrink-0` 영역으로 고정된다.
- 중앙 `레슨 콘텐츠` 영역만 `overflow-y-auto`로 스크롤되어 긴 콘텐츠가 헤더와 CTA를 밀어내지 않는다.
- 시작 화면도 `0/전체 스텝` 진행률 헤더와 고정 CTA 구조를 사용한다.

## 2026-06-15 시작 — 분류 스텝 CTA 겹침 수정

- 레슨 분류 스텝에서 하단 CTA가 `태그 선택` 패널 위를 덮어 태그를 누를 수 없는 문제를 수정한다.
- 조사 범위는 `apps/web` 레슨 분류 렌더러, 레슨 CTA 레이아웃, 회귀 테스트, 문서 갱신으로 제한한다.
- 원인 확인 후 CTA 고정 구조를 유지하면서 태그 선택 영역이 문서 흐름 안에서 하단 CTA 공간을 확보하도록 수정한다.

## 2026-06-15 완료 — 분류 스텝 CTA 겹침 수정

- 분류 스텝의 `태그 선택` 패널을 `absolute` 배치에서 일반 flex 흐름으로 되돌려 하단 CTA와 같은 하단 영역을 경쟁하지 않게 했다.
- 레슨 본문의 기존 하단 패딩이 태그 선택 패널에도 적용되어 모바일 화면에서 태그 버튼이 CTA 뒤로 들어가지 않는다.
- 회귀 테스트는 분류 스텝 태그 패널이 `absolute` class에 의존하지 않도록 검증한다.
