...
## 2026-06-15 API 실패 fallback 점검 시작

- 코스 상세와 레슨 route가 API 실패를 로컬 fallback 콘텐츠로 숨기지 않고 명확한 오류 안내를 렌더링하는지 점검한다.
- 로컬 fallback 데이터는 제품 런타임 장애 대체 경로가 아니라 테스트와 제한된 개발 확인 용도로만 남겨야 한다.

## 2026-06-15 API 실패 fallback 제거 완료

- 코스 상세 route는 코스 또는 진행 정보 API 실패 시 로컬 코스 상세/진행률 fallback을 만들지 않고 오류 안내를 렌더링한다.
- 레슨 route는 레슨 API 실패 시 로컬 레슨 fallback을 사용하지 않고 오류 안내를 렌더링한다.
- 제품 런타임에서 사용되던 코스 상세와 레슨 fallback 데이터 모듈을 제거했다.
