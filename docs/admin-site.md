...
- 저장 API와 optimistic locking은 별도 구현 전까지 현재 계약에서 제외한다.
- 레슨 요약은 고정 빈 배열이 아니라 DB의 `summary_json` 값을 editor DTO로 전달해 표시한다.

## 2026-06-15 어드민 코스 미리보기 계약 정리 완료

- Admin API의 코스 상세 editor 경계는 GET-only 읽기 계약으로 유지한다.
- Admin UI는 제목과 안내를 코스 미리보기/읽기 전용으로 바꾸고, 화면의 form control을 비활성화했다.
- `AdminCourseLessonDto`에 `summary`를 추가해 레슨 요약 미리보기가 실제 콘텐츠 값을 표시한다.

## 2026-06-15 어드민 조회 요청 상한 적용 시작

- 코스 목록, 사용자 목록, 레슨별 분석 목록의 페이지 크기 query는 과도한 DB 조회를 막기 위해 서버에서 상한을 검증한다.
- 분석 요약의 조회 기간 query는 운영 화면에서 필요한 범위 안으로 제한한다.
- 잘못된 query는 기존 오류 계약과 동일하게 `400 invalid_request`로 응답한다.

## 2026-06-15 어드민 조회 요청 상한 적용 완료

- 코스 목록, 사용자 목록, 레슨별 분석 목록의 `pageSize`는 최대 100으로 제한한다.
- 분석 요약의 `days`는 최대 365로 제한한다.
- query 검증은 어드민 API route 경계에서 수행하고, 상한을 넘는 요청은 서비스나 repository 호출 전에 `400 invalid_request`로 거절한다.
...
- 대시보드는 총 사용자, 최근 7일 활성 사용자, 오늘/최근 7일 가입, 누적 완료 레슨, 활성 코스, 활성 레슨, 최근 활동 사용자를 반환한다.
- 지표는 새 테이블 없이 기존 학습자 사용자, 프로필, 활동일, 레슨 진행, 콘텐츠 상태 테이블에서 계산한다.
- 관리자 요청은 Bearer 관리자 세션을 요구하며, 세션이 없으면 `401 unauthorized`를 반환한다.
