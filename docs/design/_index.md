...
5. `Button` 스타일 재사용 경로 이관은 `button-variants-migration-plan.md`에서 범위, 순서, 검증 기준을 확인한다.
6. 품질 기준은 `accessibility.md`, `assets.md`, `text-localization-policy.md`에서 확인한다.
...
| `button-variants-migration-plan.md`        | `buttonVariants` 이관 계획과 검증 기준을 관리한다.                                    |
