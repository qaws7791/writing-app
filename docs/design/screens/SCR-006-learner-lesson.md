...
- 쓰기 스텝은 제목 또는 프롬프트, 보조 라벨, 대상 주장, 안내, 참고 원문, 구조 가이드, 입력 영역, 글자 수 기준과 참조 답안 영역을 조건부로 표시한다.
...
- AI 코칭 생성에 실패하거나 한도를 초과해도 `피드백 없이 계속하기`를 제공한다. 서버가 학습 전이를 확정하면 레슨은 다음 스텝이나 완료 화면으로 바로 이동한다.
