...
- 진행 헤더의 막대는 완료한 스텝 비율이고, 단계 수는 현재 위치다. 아직 확인하지 않은 현재 스텝은 막대에 넣지 않는다.
- 하단 CTA는 상단 구분선 없이 본문과 이어지며 모바일 safe area inset을 반영한다. Footer 배경은 불투명 `background`다.
...
- 완료 화면은 semantic `background` 위에 `LessonComplete` anatomy를 사용한다. 큰 완료 표식과 완료한 활동 수를 두고, 진입 모션은 `prefers-reduced-motion`에서 줄인다.
...
- 읽기 출처는 외부 자료가 있을 때만 표시한다. `자체 집필`과 빈 값은 출처 행을 두지 않는다.
...
- 완료 화면의 다음 레슨 행동은 다음 레슨 시작 화면(또는 저장된 진행이 있으면 해당 스텝)으로 이동한다. Primary는 전폭이고 다음 레슨 제목을 부제로 둔다. `코스로 돌아가기`는 텍스트 행동이다.
...
- 진행 화면의 현재 스텝 제목은 `h1`이다. 시작 화면과 완료 화면도 각각 화면 `h1`을 둔다.
