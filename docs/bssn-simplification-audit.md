...
| 6        | 프로필, 검색, 레거시 리다이렉트, 장식성 XP/생명/색종이     | 제거 효과는 중간 이하이지만 작은 단순화 후보다.                                                                                     |
...
레슨 UI에는 `lives`, `xpAvailable`, `xpEarned`, `showStreak`, `correctRate`, `shareableQuote`, `celebrationStyle: "confetti"` 같은 장식성 속성이 있다. 현재 실제 성취 저장은 완료 레슨과 진행 위치 중심이다.

추천 단순화는 저장/DTO/콘텐츠 seed에서 장식성 속성을 제거하고, UI도 완료 메시지와 다음 행동만 남기는 것이다. 학습 영향은 낮고 레슨 타입과 UI 상태가 줄어든다.
...
현재 코스 수가 작다면 검색은 카테고리 탐색으로 대체할 수 있다. 제거하면 `/courses/search`, 웹 검색 진입점, 검색 DTO/테스트를 줄일 수 있다.

다만 검색 구현 자체는 스토리지나 CMS보다 작다. 코스 수가 빠르게 늘지 않는다면 지연 후보로 둔다.

### 9. 프로필 페이지 축소

현재 프로필은 완료 레슨 수와 진행 중 코스 수 요약에 가깝다. 홈에서 진행 중인 코스를 이미 보여준다면 별도 프로필 페이지는 초기 학습 흐름에 필수는 아니다.

추천 단순화는 프로필을 제거하거나 계정 메뉴의 작은 요약으로 합치는 것이다. 제거 효과는 작지만 네비게이션, `/profile`, DTO, API route를 줄인다.
...
`/home`, `/courses`, `/courses/[id]`, `/lesson`은 `/app/...`으로 리다이렉트하는 얇은 파일이다. 제거 효과는 작지만, 아직 공개 사용자가 없고 외부 링크가 없다면 단일 경로 체계만 남기는 편이 단순하다.
...
현재 가장 명확한 BSSN 단순화는 썸네일 업로드 제거다. 이 하나만으로 Docker, RustFS, S3 SDK, 스토리지 환경 변수, 공개 버킷 초기화, 배포 스토리지 서비스를 없앨 수 있고 학습 품질 손실은 거의 없다.

이번 계획은 어드민 코스 편집기, 레슨 스텝 타입, Storybook/UI 축소를 제외하고도 충분히 큰 단순화 효과를 낸다. 남은 핵심 결정은 단일 현재 커리큘럼으로 갈 수 있는지, 그리고 인증 경로를 Google 하나로 고정할 수 있는지다.
