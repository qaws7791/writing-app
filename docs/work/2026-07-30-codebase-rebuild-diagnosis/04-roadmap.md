...
이 열은 진단 시점 기준이다. M0에서 운영 단계가 실사용자 0의 초기 개발 단계로 확정되어 Step 5·8·9의 "필요"는 스테이징 실데이터 검증이 아니라 시드·E2E 픽스처 기반 로컬 검증으로 대체한다. 위험 등급 자체는 낮아지지 않는다 — [`progress.md`](./progress.md) 의 해당 항목을 따른다.
