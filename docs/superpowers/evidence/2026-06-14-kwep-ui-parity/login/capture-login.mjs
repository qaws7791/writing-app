...
const outDir =
  "/Users/mac/github/writing-app/docs/superpowers/evidence/2026-06-14-kwep-ui-parity/login"
const chromePath =
  "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"

const targets = [
  { name: "kwep", url: "http://127.0.0.1:5173/login" },
  { name: "product", url: "http://localhost:3000/login" },
]
...
const collectPage = (props) => {
  const root =
    Array.from(document.querySelectorAll("div")).find((element) =>
      (element.getAttribute("class") ?? "").includes("min-h-[80vh]")
    ) ?? document.body
...
const verifyClick = async (page) => {
  await page.setViewportSize({ height: 844, width: 390 })
  await page.goto("http://localhost:3000/login?next=/app/courses", {
    waitUntil: "networkidle",
  })
  await page.waitForTimeout(700)
  await page
    .getByRole("button", { exact: true, name: "Google로 계속하기" })
    .click()
...
const clickResult = await verifyClick(page)
await writeFile(
  `${outDir}/interaction-latest.json`,
  JSON.stringify(
    {
      googleButton: {
        expectedPrefix: "/api/auth/sign-in/google",
        productPath: clickResult,
      },
    },
    null,
    2
