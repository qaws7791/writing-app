...
const outDir =
  "/Users/mac/github/writing-app/docs/superpowers/evidence/2026-06-14-kwep-ui-parity/home"
const chromePath =
...
const verifyClick = async (target) => {
  const context = await browser.newContext({
    deviceScaleFactor: 1,
  ...
  await page.evaluate(() => document.fonts.ready)
  await page.waitForTimeout(700)
  await page.getByText("코스 둘러보기", { exact: true }).click()
...
const clickResult = Object.fromEntries(
  await Promise.all(
    targets.map(async (target) => [target.name, await verifyClick(target)])
  )
)
