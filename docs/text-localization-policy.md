...
- 학습자 API, 관리자 API, core service, DB repository의 주요 오류 응답 메시지를 한국어로 정리했다.
- 웹/어드민 클라이언트의 알 수 없는 오류 fallback 메시지를 한국어로 정리했다.
- 공유 UI 컴포넌트의 기본 접근성 텍스트를 한국어로 정리했다.
- 생성된 OpenAPI 문서와 API 타입에 포함된 오류 예시 메시지를 한국어로 정리했다.
