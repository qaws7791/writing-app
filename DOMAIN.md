...
현재 구현은 학습자 코스 상세 화면에서 업그레이드 가능한 published 버전이 있을 때만 공지를 표시한다. 학습자가 업그레이드를 선택하면 active 마이그레이션 맵을 기준으로 완료 레슨만 새 버전으로 이전하고, 나중에 결정하면 해당 사용자와 버전 쌍에 대해 공지를 숨긴다.

## 현재 구현 상태

현재 저장소는 커리큘럼 버전 스키마, 학습 진행 버전 귀속, 버전 인식 읽기 경로, 커리큘럼 노드 상태 읽기 정책, 관리자 draft/published 발행 워크플로우, 커리큘럼 마이그레이션 맵, 학습자 업그레이드 공지와 선택 UX를 구현했다. 공개 콘텐츠와 학습 진행 경로는 active 노드만 신규 학습 경로로 사용하고, 관리자 코스 트리 조회는 최신 published 버전의 active/deprecated/archived 상태를 표시한다. 관리자는 최신 published snapshot에서 draft를 만들고 draft를 published로 승격할 수 있으며, 버전 사이의 레슨 매핑을 명시해 사용자 진행을 새 버전에 적용할 수 있다. 학습자는 코스 상세 화면에서 새 커리큘럼 공지를 확인하고 직접 업그레이드하거나 나중에 결정할 수 있다.

커리큘럼 변경 안전장치의 현재 로드맵은 8개 단계를 모두 구현한 상태다.
